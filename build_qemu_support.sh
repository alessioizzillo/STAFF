...
if [ "$ARGS" = "qemu_full" ] || [ "$ARGS" = "all" ]; then

    echo ""
    echo -e "\033[33m[*]\033[0m Starting QEMU System Mode (FULL) compilation..!"
    ...
    cd qemu_mode_full/DECAF_qemu_2.10 || exit 1

    ./configure --target-list=mipsel-softmmu,mips-softmmu,arm-softmmu --static --disable-werror --python=/usr/bin/python2
