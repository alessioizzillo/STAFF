                   ...
                   int rt, int base, int16_t offset)
{
    ...
    case OPC_LWLE:
        ...
        /* fall through */
    case OPC_LWL:
        gen_helper_skip_taint_left_shift_amount_enter(cpu_env);
        t1 = tcg_temp_new();
        /* Do a byte access to possibly trigger a page
           fault with the unaligned address.  */
        tcg_gen_qemu_ld_tl(t1, t0, mem_idx, MO_UB);
        ...
        gen_store_gpr(t0, rt);
        gen_helper_skip_taint_left_shift_amount_exit(cpu_env);
        break;
    case OPC_LWRE:
        mem_idx = MIPS_HFLAG_UM;
        /* fall through */
    case OPC_LWR:
        gen_helper_skip_taint_right_shift_amount_enter(cpu_env);
        t1 = tcg_temp_new();
        /* Do a byte access to possibly trigger a page
           fault with the unaligned address.  */
        tcg_gen_qemu_ld_tl(t1, t0, mem_idx, MO_UB);
        ...
        tcg_gen_ext32s_tl(t0, t0);
        gen_store_gpr(t0, rt);
        gen_helper_skip_taint_right_shift_amount_exit(cpu_env);
        break;
    case OPC_LLE:
        mem_idx = MIPS_HFLAG_UM;
        /* fall through */
...
#ifdef CONFIG_TCG_TAINT    
    shadow_arg[bcond] = tcg_global_mem_new(cpu_env, offsetof(CPUMIPSState, taint_bcond), "taint_bcond");
#endif /* CONFIG_TCG_TAINT */
...
#ifdef CONFIG_TCG_TAINT
    for (i = 1; i < 32; i++) {
      taint_cpu_gpr[i] = tcg_global_mem_new(cpu_env,
        offsetof(CPUMIPSState, active_tc.taint_gpr[i]), taint_regnames[i]);
      shadow_arg[(int)cpu_gpr[i]] = taint_cpu_gpr[i];
    }
    taint_cpu_PC = tcg_global_mem_new(cpu_env,
      offsetof(CPUMIPSState, active_tc.taint_PC), "taint_PC");
    shadow_arg[(int)cpu_PC] = taint_cpu_PC;

    tempidx = tcg_global_mem_new_i32(cpu_env,
      offsetof(CPUMIPSState, tempidx), "tempidx");
    tempidx2 = tcg_global_mem_new_i32(cpu_env,
      offsetof(CPUMIPSState, tempidx), "tempidx2");

    int off = tcg_ctx.nb_temps;

    taint_temp_names = malloc(((TCG_MAX_TEMPS-off)/2) * sizeof(char *));
    if (taint_temp_names == NULL) {
        fprintf(stderr, "Memory allocation failed!\n");
        return 1;
    }
    
    for (i = 0; i < (TCG_MAX_TEMPS-off)/2; i++) {
        taint_temp_names[i] = malloc(20 * sizeof(char));
        if (taint_temp_names[i] == NULL) {
            fprintf(stderr, "Memory allocation failed for taint_temp_names[%d]!\n", i);
            return 1;
        }
        snprintf(taint_temp_names[i], 20, "taint_tmp%d", i);
    }

    for (i = 0; i < (TCG_MAX_TEMPS-off)/2; i++) {
        shadow_arg[(TCG_MAX_TEMPS+off)/2+i] = tcg_global_mem_new_i32(cpu_env,
      offsetof(CPUMIPSState, active_tc.taint_temps[i]), taint_temp_names[i]);
    }
...
#ifdef TARGET_MIPS
#ifdef CONFIG_TCG_TAINT
target_ulong check_registers_taint(CPUArchState *env) {
	target_ulong taint_status = 0;
    int i;
    for (i = 1; i < 32; i++) {
        taint_status |= env->active_tc.taint_gpr[i];
    }
