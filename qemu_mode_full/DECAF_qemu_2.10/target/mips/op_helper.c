...
void helper_skip_taint_right_shift_amount_enter(CPUMIPSState *env) {
}

void helper_skip_taint_right_shift_amount_exit(CPUMIPSState *env) {
}

void helper_skip_taint_left_shift_amount_enter(CPUMIPSState *env) {
}

void helper_skip_taint_left_shift_amount_exit(CPUMIPSState *env) {
}

void helper_swl(CPUMIPSState *env, target_ulong arg1, target_ulong arg2,
                int mem_idx)
{
    target_long old_tempidx = env->tempidx;

    env->tempidx = old_tempidx >> 24;
    do_sb(env, arg2, (uint8_t)(arg1 >> 24), mem_idx, GETPC());

    if (GET_LMASK(arg2) <= 2) {
        env->tempidx = old_tempidx >> 16;
        do_sb(env, GET_OFFSET(arg2, 1), (uint8_t)(arg1 >> 16), mem_idx,
              GETPC());
    }

    if (GET_LMASK(arg2) <= 1) {
        env->tempidx = old_tempidx >> 8;
        do_sb(env, GET_OFFSET(arg2, 2), (uint8_t)(arg1 >> 8), mem_idx,
              GETPC());
    }

    if (GET_LMASK(arg2) == 0) {
        env->tempidx = old_tempidx;
