...
void DECAF_blocks_init(int debug)
{
	...
	BlockBackend *blk;
		...
		if (index == 0){
			...
			DECAF_bdrv_open(index,(void *)blk,debug);	
		}
		++index; 
...
void DECAF_bdrv_open(int index, void *opaque, int debug) {
  ...
  else
  {
    ...
    process_fs_info(disk_info_internal[0].fs, debug);
