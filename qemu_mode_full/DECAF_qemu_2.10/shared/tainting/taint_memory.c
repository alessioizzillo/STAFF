...
void allocate_taint_memory_page_table(void) {
  ...
  taint_memory_page_table = (tbitpage_middle_t **) malloc(taint_memory_page_table_root_size * sizeof(void*));
  allocate_leaf_pool();
...
void  __taint_ldb_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end)
{
	  ...
	  if (leaf_node) {
	    cpu_single_env->tempidx = ldub_p((void *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
...
void  __taint_ldw_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end)
{	
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
#if defined(TARGET_MIPS)
	unsigned int middle_node_index;
	  ...
	  if (leaf_node) {
		if (!end)
	    	cpu_single_env->tempidx = lduw_le_p((void *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
		else
	    	cpu_single_env->tempidx = lduw_be_p((void *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
...
void  __taint_ldl_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end)
{
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
#if defined(TARGET_MIPS)
	 unsigned int middle_node_index;
	  ...
	  if (leaf_node) {
		if (!end)
	    	cpu_single_env->tempidx = ldl_le_p((void *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
		else
	    	cpu_single_env->tempidx = ldl_be_p((void *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
...
void  __taint_ldq_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end)
{
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
#if defined(TARGET_MIPS)
	  unsigned int middle_node_index;
	  ...
	  if (leaf_node) {
		if (!end)
	    	cpu_single_env->tempidx = ldl_le_p((uint32_t *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
		else
	    	cpu_single_env->tempidx = ldl_be_p((uint32_t *)leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
#if TCG_TARGET_REG_BITS == 64
	    cpu_single_env->tempidx = cpu_single_env->tempidx & 0xFFFFFFFF;
#endif /* TCG_TARGET_REG_BITS == 64 */
	  }
	  if (leaf_node2) {
		if (!end) {
#if TCG_TARGET_REG_BITS == 64
	    	cpu_single_env->tempidx |= ldl_le_p((uint32_t *)(leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)));
#else
	    	cpu_single_env->tempidx2 = ldl_le_p((uint32_t *)(leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)));
#endif /* TCG_TARGET_REG_BITS == 64 */
		}
		else {
#if TCG_TARGET_REG_BITS == 64
	    	cpu_single_env->tempidx |= ldl_be_p((uint32_t *)(leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)));
#else
	    	cpu_single_env->tempidx2 = ldl_be_p((uint32_t *)(leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)));
#endif /* TCG_TARGET_REG_BITS == 64 */
		}
	  }
...
void  __taint_ldb_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldb_raw_paddr(d,addr,vaddr,end);
}

void  __taint_ldw_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldw_raw_paddr(d,addr,vaddr,end);
}

void  __taint_ldl_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldl_raw_paddr(d,addr,vaddr,end);
}

void  __taint_ldq_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_ldq_raw_paddr(d,addr,vaddr,end);
}

void  __taint_stb_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end) {
	...
	if (leaf_node) {
		...
		stb_p((uint8_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), cpu_single_env->tempidx);
...
void  __taint_stw_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end) {
	if (!taint_memory_page_table || addr >= ram_size)
		return;
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
#if defined(TARGET_MIPS)
  ...
  char changed = 0;
	...
	if (leaf_node) {
		...
		if (!end)
			stw_le_p((uint16_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), (uint16_t)cpu_single_env->tempidx);
		else
	    	stw_be_p((uint16_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), (uint16_t)cpu_single_env->tempidx);
...
void  __taint_stl_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end) {
	if (!taint_memory_page_table || addr >= ram_size)
		return;
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
#if defined(TARGET_MIPS)
	...
	if (leaf_node) {
		...
		if (!end)
			stl_le_p((uint32_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), cpu_single_env->tempidx);
		else
	    	stl_be_p((uint32_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), cpu_single_env->tempidx);
...
void  __taint_stq_raw_paddr(int d, ram_addr_t addr,gva_t vaddr, uint8_t end) {
	if (!taint_memory_page_table || addr >= ram_size)
		return;
	CPUArchState *cpu_single_env = (CPUArchState *)current_cpu->env_ptr;
	/* AWH - Keep track of whether the taint state has changed for this location.
	...
	if (leaf_node) {
		if (!end)
			stl_le_p((uint32_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), (cpu_single_env->tempidx & 0xFFFFFFFF));
		else
	    	stl_be_p((uint32_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK)), (cpu_single_env->tempidx & 0xFFFFFFFF));
	}
	if (leaf_node2) {
		if (!end)
			stl_le_p((uint32_t *) (leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)), ((cpu_single_env->tempidx & 0xFFFFFFFF) >> 32));
		else
	    	stl_be_p((uint32_t *) (leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)), ((cpu_single_env->tempidx & 0xFFFFFFFF) >> 32));
	}
#else
	...
	if (leaf_node2) {
		before = *(uint32_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
		if (!end)
			stl_le_p((uint32_t *) (leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)), cpu_single_env->tempidx2);
		else
	    	stl_be_p((uint32_t *) (leaf_node2->bitmap + ((addr+4) & LEAF_ADDRESS_MASK)), cpu_single_env->tempidx2);
		after = *(uint32_t *) (leaf_node->bitmap + (addr & LEAF_ADDRESS_MASK));
...
void  __taint_stb_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stb_raw_paddr(d, addr, vaddr, end);
}

void  __taint_stw_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stw_raw_paddr(d, addr, vaddr, end);
}

void  __taint_stl_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stl_raw_paddr(d, addr, vaddr, end);
}

void  __taint_stq_raw(int d, unsigned long addr, gva_t vaddr, uint8_t end) {
	addr = qemu_ram_addr_from_host_nofail((void*)addr);
	__taint_stq_raw_paddr(d, addr, vaddr, end);
}
