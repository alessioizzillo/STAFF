               ...
               )
{
  ...
  info_return->handle = DECAF_registerOptimizedBlockBeginCallback(&hookapi_check_hook, NULL, pc, OCB_CONST);
...
int hookapi_hook_all_module_functions(const char *proc_name, const char *mod_name, int is_global, 
                                      target_ulong cr3, uint32_t pid, hook_proc_t fnhook, int debug)
{
    target_ulong base;
    ...
    tsk_fs_file_walk(file_fs, TSK_FS_FILE_WALK_FLAG_NONE, write_action, &local_copy);
    
    std::istringstream is(local_copy);
    elfio reader;
    bool header_present = reader.load(is);
    if (!header_present) {
        fprintf(stderr, "Error loading ELF file for module: %s\n", mod_name);
        return -1;
    }
        ...
        if (psec->get_type() == SHT_DYNSYM || psec->get_type() == SHT_SYMTAB) {
            ...
            for (unsigned int j = 0; j < symbols.get_symbols_num(); ++j) {
                ...
                if (type == STT_FUNC) {
                    ...
                    hookapi_hook_function(is_global, value, cr3, fnhook, info, sizeof(func_info_enter));

                    if (debug) {
                        fprintf(fp, "mod_name=\"%s\" elf_name=\"%s\" base_addr=\"%lx\" func_addr=\"%lx\"\n",
...
void load_module(target_ulong cr3, const char *mod_name, uint32_t pid) {
  target_ulong base;
  module *mod = VMI_find_module_by_name(mod_name, cr3, &base);
  add_base_addr(pid, mod->inode_number, base, std::string(mod_name));
}

void copy_inode_trace_to_shmem_wrapper(int pid, char proc_name[MAX_MODULE_NAME_LENGTH]) {
  copy_inode_trace_to_shmem(pid, proc_name, cur_crashes);
}

int get_pc_text_info_wrapper(int pid, uintptr_t pc, int *adjusted_pc, uint64_t *inode_num) {
  if (!adjusted_pc || !inode_num) {
    return -1;
  }
  ...
  if (!get_pc_text_info(pid, pc, adj_pc, inode)) {
    return -1;
  }

  *adjusted_pc = adj_pc;
  *inode_num = static_cast<uint64_t>(inode);
