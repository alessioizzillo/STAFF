                         ...
                         target_ulong addr, uintptr_t retaddr, int size)
{
    ...
    if (taint_tracking_enabled) {
        switch (size) {
            case 1:
                __taint_ldb_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            case 2:
                __taint_ldw_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            case 4:
                __taint_ldl_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            case 8:
                __taint_ldq_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            default:
                assert(0);
        }
...
#ifdef TARGET_MIPS
	if (taint_tracking_enabled && !second_ccache_flag && env->tempidx){
        CPUState *cpu = ENV_GET_CPU(env);
                      ...
                      uintptr_t retaddr, int size)
{
    ...
    if (taint_tracking_enabled) {
        switch (size) {
            case 1:
                __taint_stb_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            case 2:
                __taint_stw_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            case 4:
                __taint_stl_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            case 8:
                __taint_stq_raw_paddr(0, physaddr, cpu->mem_io_vaddr);
                break;
            default:
                assert(0);
        }
