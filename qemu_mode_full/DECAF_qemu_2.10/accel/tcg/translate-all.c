...
void tb_lock(void)
{
    ...
    if (!second_ccache_flag) {
        ...
        qemu_mutex_lock(&tcg_ctx.tb_ctx2.tb_lock);
        have_tb_lock2++;        
    }
}
...
void tb_unlock(void)
{
    ...
    if (!second_ccache_flag) {
        ...
        have_tb_lock2--;
        qemu_mutex_unlock(&tcg_ctx.tb_ctx2.tb_lock);        
    }
}
...
uintptr_t _tb_find_pc(CPUState *cpu, uintptr_t tc_ptr) {
    TranslationBlock *local_tb = tb_find_pc(tc_ptr);
    uintptr_t searched_pc = tc_ptr;
    target_ulong data[TARGET_INSN_START_WORDS] = { local_tb->pc };
    uintptr_t host_pc = (uintptr_t)local_tb->tc_ptr;
    CPUArchState *env = cpu->env_ptr;
    uint8_t *p = local_tb->tc_search;
    int i, j, num_insns = local_tb->icount;
...
static inline void code_gen_alloc(size_t tb_size)
{
    ...
    /* size this conservatively -- realloc later if needed */
    tcg_ctx.tb_ctx2.tbs_size =
        tcg_ctx.code_gen_buffer_size / CODE_GEN_AVG_BLOCK_SIZE / 8;
    if (unlikely(!tcg_ctx.tb_ctx2.tbs_size)) {
        tcg_ctx.tb_ctx2.tbs_size = 64 * 1024;
    }
    tcg_ctx.tb_ctx2.tbs = g_new(TranslationBlock *, tcg_ctx.tb_ctx2.tbs_size);

    qemu_mutex_init(&tcg_ctx.tb_ctx2.tb_lock);
}
...
static void tb_htable_init(void)
{
    ...
    qht_init(&tcg_ctx.tb_ctx2.htable, CODE_GEN_HTABLE_SIZE, mode);
}
...
void tb_free(TranslationBlock *tb)
{
    ...
    if (!second_ccache_flag) {
        ...
        if (tcg_ctx.tb_ctx2.nb_tbs > 0 &&
                tb == tcg_ctx.tb_ctx2.tbs[tcg_ctx.tb_ctx2.nb_tbs - 1]) {
            size_t struct_size = ROUND_UP(sizeof(*tb), qemu_icache_linesize);

            tcg_ctx.code_gen_ptr = tb->tc_ptr - struct_size;
            tcg_ctx.tb_ctx2.nb_tbs--;
        }       
    }
}
...
static void do_tb_flush(CPUState *cpu, run_on_cpu_data tb_flush_count)
{
    ...
    if (!second_ccache_flag) {
        ...
        if (tcg_ctx.tb_ctx2.tb_flush_count != tb_flush_count.host_int) {
            goto done;
        }
    } 
    ...
    if (!second_ccache_flag)
        ...
        printf("qemu: flush code_size=%ld nb_tbs=%d avg_tb_size=%ld\n",
            (unsigned long)(tcg_ctx.code_gen_ptr - tcg_ctx.code_gen_buffer),
            tcg_ctx.tb_ctx2.nb_tbs, tcg_ctx.tb_ctx2.nb_tbs > 0 ?
            ((unsigned long)(tcg_ctx.code_gen_ptr - tcg_ctx.code_gen_buffer)) /
            tcg_ctx.tb_ctx2.nb_tbs : 0);
#endif
    ...
    if (!second_ccache_flag) {
        ...
        tcg_ctx.tb_ctx2.nb_tbs = 0;
        qht_reset_size(&tcg_ctx.tb_ctx2.htable, CODE_GEN_HTABLE_SIZE);
    }
    page_flush_tb();
    ...
    if (!second_ccache_flag) {
        ...
        atomic_mb_set(&tcg_ctx.tb_ctx2.tb_flush_count,
                    tcg_ctx.tb_ctx2.tb_flush_count + 1);        
    }
...
void tb_flush(CPUState *cpu)
{
    if (tcg_enabled()) {
        if (!second_ccache_flag) { 
            ...
            unsigned tb_flush_count = atomic_mb_read(&tcg_ctx.tb_ctx2.tb_flush_count);
            async_safe_run_on_cpu(cpu, do_tb_flush,
                                RUN_ON_CPU_HOST_INT(tb_flush_count));            
        }
    }
...
static void tb_invalidate_check(target_ulong address)
{
    ...
    if (!second_ccache_flag)
        ...
        qht_iter(&tcg_ctx.tb_ctx2.htable, do_tb_invalidate_check, &address);    
}
...
static void tb_page_check(void)
{
    if (!second_ccache_flag)
        ...
        qht_iter(&tcg_ctx.tb_ctx2.htable, do_tb_page_check, NULL);    
}
...
void tb_phys_invalidate(TranslationBlock *tb, tb_page_addr_t page_addr)
{
    ...
    if (!second_ccache_flag)
        ...
        qht_remove(&tcg_ctx.tb_ctx2.htable, tb, h);    
    ...
    if (!second_ccache_flag)
        ...
        tcg_ctx.tb_ctx2.tb_phys_invalidate_count++;    
}
                         ...
                         tb_page_addr_t phys_page2)
{
    ...
    if (!second_ccache_flag)
        ...
        qht_insert(&tcg_ctx.tb_ctx2.htable, tb, h);    
...
void mod_tb_gen_code(CPUState *cpu, TranslationBlock *tb)
{
    CPUArchState *env = cpu->env_ptr;
 ...
 buffer_overflow:
    ...
    gen_code_size = tcg_gen_code(&tcg_ctx, tb->tc_ptr);
...
/* Called with mmap_lock held for user mode emulation.  */
TranslationBlock *tb_gen_code(long insn_index, CPUState *cpu,
                              ...
                              uint32_t flags, int cflags)
{
 ...
 buffer_overflow:
    ...
    if (insn_index == -1)
        ...
        gen_code_size = tcg_gen_code_search_pc(&tcg_ctx, tb->tc_ptr, insn_index);
                                   ...
                                   int is_cpu_write_access)
{
    ...
    if (current_tb_modified) {
        /* we generate a block containing just the instruction
           ...
           itself */
        tb_gen_code(-1, cpu, current_pc, current_cs_base, current_flags, 1);
...
tb_find_tb_ctx2:
        if (tc_ptr < tcg_ctx.tb_ctx2.tbs[0]) {
            goto tb_find_tb_ctx;
        }
        ...
        m_max = tcg_ctx.tb_ctx2.nb_tbs - 1;

        while (m_min <= m_max) {
            m = (m_min + m_max) >> 1;

            tb = tcg_ctx.tb_ctx2.tbs[m];
        ...
        return tcg_ctx.tb_ctx2.tbs[m_max];
    }
}
...
void cpu_io_recompile(CPUState *cpu, uintptr_t retaddr)
{
    ...
    tb_gen_code(-1, cpu, pc, cs_base, flags, cflags);
...
void dump_exec_info(FILE *f, fprintf_function cpu_fprintf)
{
    ...
    direct_jmp_count = 0;
    direct_jmp2_count = 0;
    int n = !second_ccache_flag ? tcg_ctx.tb_ctx.nb_tbs : tcg_ctx.tb_ctx2.nb_tbs;
    for (i = 0; i < n; i++) {
        if (!second_ccache_flag)
            tb = tcg_ctx.tb_ctx.tbs[i];
        else
            tb = tcg_ctx.tb_ctx2.tbs[i];        
    ...
    if (!second_ccache_flag) {
        ...
        cpu_fprintf(f, "TB count            %d\n", tcg_ctx.tb_ctx2.nb_tbs);
        cpu_fprintf(f, "TB avg target size  %d max=%d bytes\n",
                tcg_ctx.tb_ctx2.nb_tbs ? target_code_size /
                        tcg_ctx.tb_ctx2.nb_tbs : 0,
                max_target_code_size);
        cpu_fprintf(f, "TB avg host size    %td bytes (expansion ratio: %0.1f)\n",
                tcg_ctx.tb_ctx2.nb_tbs ? (tcg_ctx.code_gen_ptr -
                                        tcg_ctx.code_gen_buffer) /
                                        tcg_ctx.tb_ctx2.nb_tbs : 0,
                    target_code_size ? (double) (tcg_ctx.code_gen_ptr -
                                                tcg_ctx.code_gen_buffer) /
                                                target_code_size : 0);
        cpu_fprintf(f, "cross page TB count %d (%d%%)\n", cross_page,
                tcg_ctx.tb_ctx2.nb_tbs ? (cross_page * 100) /
                                        tcg_ctx.tb_ctx2.nb_tbs : 0);
        cpu_fprintf(f, "direct jump count   %d (%d%%) (2 jumps=%d %d%%)\n",
                    direct_jmp_count,
                    tcg_ctx.tb_ctx2.nb_tbs ? (direct_jmp_count * 100) /
                            tcg_ctx.tb_ctx2.nb_tbs : 0,
                    direct_jmp2_count,
                    tcg_ctx.tb_ctx2.nb_tbs ? (direct_jmp2_count * 100) /
                            tcg_ctx.tb_ctx2.nb_tbs : 0);
    }
    ...
    if (!second_ccache_flag)
        ...
        qht_statistics_init(&tcg_ctx.tb_ctx2.htable, &hst);    
    ...
    if (!second_ccache_flag) {
        ...
        cpu_fprintf(f, "TB flush count      %u\n",
                atomic_read(&tcg_ctx.tb_ctx2.tb_flush_count));
        cpu_fprintf(f, "TB invalidate count %d\n",
                tcg_ctx.tb_ctx2.tb_phys_invalidate_count);
        cpu_fprintf(f, "TLB flush count     %d\n", tlb_flush_count);        
    }
