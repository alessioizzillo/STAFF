        ...
        if (cpu_watchpoint_address_matches(wp, vaddr, len)
            ...
            if (!cpu->watchpoint_hit) {
                ...
                if (wp->flags & BP_STOP_BEFORE_ACCESS) {
                    ...
                    tb_gen_code(-1, cpu, pc, cs_base, cpu_flags, 1);
    ...
    for (;;) {
        if (!memory_access_is_direct(mr, true)) {
            ...
            if (taint_tracking_enabled) {
                if (memmem(buf, l, "GET", 3) || memmem(buf, l, "POST", 4)) {
                    if ((exec_mode == RUN || exec_mode == PREANALYSIS) && send_next_region && *send_next_region == 1) {
                        *send_next_region = 0;
                        ...
                        if (first_write) {
                            if (debug)
                                fp = fopen("debug/received_requests.log", "w");
                            first_write = 0;
                        } else {
                            sink_id++;
                            if (debug)
                                fp = fopen("debug/received_requests.log", "a+");
                        }
                    ...
                    uint8_t *taint_status = (uint8_t *)malloc(l);
                    memset(taint_status, 0x0, l);
