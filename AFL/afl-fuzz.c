...
EXP_ST u32 child_tmout_secs = 1;      /* Configurable child timeout (s)   */
...
void *send_requests(void *arg) {
  ...
  while (!file_exists("fuzzing_started")) {
    ...
    if (fread(request, 1, seed_size, seed) != seed_size) {
    ...
    if (inet_pton(AF_INET, net_ip, &server_addr.sin_addr) <= 0) {
    ...
    struct timeval timeout = { .tv_sec = child_tmout_secs, .tv_usec = 0 };
    setsockopt(sockfd, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));

    ssize_t total_sent = 0;
    while (total_sent < seed_size) {
      ssize_t sent = net_send(sockfd, timeout, request, seed_size);
      if (sent <= 0) {
        perror("send failed");
        break;
      }
    ...
    if (debug) {
      FILE *log = fopen("debug/send.log", "a");
      if (log) {
        fprintf(log, "Iteration %d: Sent %ld bytes to %s\n", iteration, total_sent, net_ip);
        fclose(log);
      }
    }
...
EXP_ST void init_forkserver(char** argv) {
  ...
  remove("fuzzing_started");

  pthread_t request_thread;
  pthread_create(&request_thread, NULL, send_requests, NULL);
  ...
  if (WIFSIGNALED(*child_retval)) {
    ...
    FATAL("Fork server crashed with signal %d", WTERMSIG(*child_retval));
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    tv.tv_sec = dry_run_phase ? child_tmout_secs : max_tmout_secs;
    tv.tv_usec = 0;

    struct timeval check_0, check_1;

    gettimeofday(&check_0, NULL);

    FD_ZERO(&read_fds);
    FD_SET(fsrv_st_fd, &read_fds);

    int res = select(fsrv_st_fd + 1, &read_fds, NULL, NULL, &tv);

    if (res > 0) {
      if (read(fsrv_st_fd, &status, 4) != 4) {
        PFATAL("Unable to communicate with fork server (OOM?)\n");
      }
    } else if (res == 0) {  
      // WARNF("ALRM!");
    ...
    gettimeofday(&check_1, NULL);

    if (dry_run_phase) {
      u32 res_time = ((check_1.tv_sec - check_0.tv_sec)) + ((check_1.tv_usec - check_0.tv_usec) / 1000000) + 1;
...
static void write_stats_file(double bitmap_cvg, double stability, double eps) {
  ...
  fprintf(f, "start_time             : %llu\n"
             ...
             orig_cmdline, slowest_exec_ms, total_cal_cycles, deduplicated_crashes, filtered_crashes);
             /* ignore errors */
...
int main(int argc, char** argv) {
  ...
  if (setenv("EXEC_MODE", "TRIFORCE", 1) != 0) {
    perror("setenv");
    return EXIT_FAILURE;
  }

  shm_unlink(SHARED_CHILD_RETVAL);
  int shm_fd = shm_open(SHARED_CHILD_RETVAL, O_CREAT | O_RDWR, 0666);
  if (shm_fd == -1) {
      perror("shm_open");
      exit(EXIT_FAILURE);
  }
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:Qw:VD:y:")) > 0)
