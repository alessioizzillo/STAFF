...
#define ck_read(fd, buf, len, fn) do { \
    if (len <= 0) break; \
    int _fd = (fd); \
    s32 _read = 0, _off = 0, _len = (s32)(len); \
    do { \
      s32 _res = read(_fd, (buf) + _off, _len); \
      if (_res != _len && (_res > 0 && _read + _res != _len)) { \
        if (_res > 0) { \
          _read += _res; \
          _len -= _res; \
          _off += _res; \
        } else { \
          RPFATAL(_res, "Short read from %s, fd %d (%d of %d bytes)", fn, _fd, \
                  _res, _len); \
        } \
      } else { \
        break; \
