...
if [ ${BASENAME} = "run.sh" ]; then
    MODE=run
elif [[ "${BASENAME}" == *"triforce"* ]]; then
    suffix=$(echo "${BASENAME}" | sed -n 's/.*triforce\([^\.]*\)\.sh/\\1/p')
    MODE="triforce${suffix}"
elif [[ "${BASENAME}" == *"staff_base"* ]]; then
    suffix=$(echo "${BASENAME}" | sed -n 's/.*staff_base\([^\.]*\)\.sh/\\1/p')
    MODE="staff_base${suffix}"
elif [[ "${BASENAME}" == *"staff_state_aware"* ]]; then
    suffix=$(echo "${BASENAME}" | sed -n 's/.*staff_state_aware\([^\.]*\)\.sh/\\1/p')
    MODE="staff_state_aware${suffix}"
elif [[ "${BASENAME}" == *"aflnet_base"* ]]; then
    suffix=$(echo "${BASENAME}" | sed -n 's/.*aflnet_base\([^\.]*\)\.sh/\\1/p')
    MODE="aflnet_base${suffix}"
...
if [ "$RUN" = "0" ]; then
    echo "%(QEMU_ENV_VARS)s  ${QEMU} ${QEMU_BOOT} -m 1024 -M ${QEMU_MACHINE} -kernel ${KERNEL} \
        %(QEMU_DISK)s \\
    ...
    echo "root=${QEMU_ROOTFS} console=ttyS0 nandsim.parts=64,64,64,64,64,64,64,64,64,64 %(QEMU_INIT)s rw debug ignore_loglevel print-fatal-signals=1 FIRMAE_NET=${FIRMAE_NET} FIRMAE_NVRAM=${FIRMAE_NVRAM} FIRMAE_KERNEL=${FIRMAE_KERNEL} FIRMAE_ETC=${FIRMAE_ETC} ${QEMU_DEBUG} nokaslr norandmaps libata.force=noncq libata.force=pio4" > afl-qemu-system-trace_cmd_append
...
def startNetwork(network, iid):
    ...
    if "run" == MODE:
        mode_abbr = "run"
    elif "staff_base" in MODE:
        suffix = MODE.split("staff_base", 1)[1]
        mode_abbr = f"sb{suffix}"
    elif "staff_state_aware" in MODE:
        suffix = MODE.split("staff_state_aware", 1)[1]
        mode_abbr = f"ss{suffix}"
    elif "triforce" in MODE:
        suffix = MODE.split("triforce", 1)[1]
        mode_abbr = f"t{suffix}"
    elif "aflnet_base" in MODE:
        suffix = MODE.split("aflnet_base", 1)[1]
        mode_abbr = f"ab{suffix}"
