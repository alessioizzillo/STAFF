...
function cleanup_on_exit() {
    if egrep -sqi "true" "${WORK_DIR}/web_check"; then
        ...
        rm -f ./images/${MODE}/${IID}.tar.gz || true
        rm -f ./images/${MODE}/${IID}.kernel || true
        rm -f "${FIRMWARE}" || true
        rm -rf "${WORK_DIR}" || true
        sed -i '$ d' "${csv_file_path}" || true
        exit 1
    fi

    if [ -f "time_web_constraint" ]; then
        TIME_WEB=$(cat "${WORK_DIR}/time_web")
        TIME_WEB_CONSTRAINT=$(cat "time_web_constraint")
        if (( $(echo "$TIME_WEB >= $TIME_WEB_CONSTRAINT" | bc -l) )); then
            echo "Time ${TIME_WEB} exceeds constraint ${TIME_WEB_CONSTRAINT}!"
            rm -f ./images/${MODE}/${IID}.tar.gz || true
            rm -f ./images/${MODE}/${IID}.kernel || true
            rm -f "${FIRMWARE}" || true
            rm -rf "${WORK_DIR}" || true
...
function run_emulation()
{
    ...
    csv_file_path="/STAFF/FirmAE/firm_db_${MODE}.csv"
    ...
    if [ -f "arch_constraints" ]; then
        if ! grep -Ff arch_constraints <(echo "${ARCH}"); then
            echo "Architecture ${ARCH} not allowed by constraints!"
            rm ./images/${MODE}/$IID.tar.gz || true
            rm ./images/${MODE}/$IID.kernel || true
            rm ${FIRMWARE} || true
            rm -r ${WORK_DIR} || true
            sed -i '$ d' ${csv_file_path} || true
            exit 1
        ...
        echo "arch_constraints file not found. Allowing architecture ${ARCH}."
    fi
