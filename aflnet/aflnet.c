...
int handle_http_protocol(char *buf, int n, char **response_buf, unsigned int *len, int *state, int *remaining_bytes, int *is_chunked, char **content_type) {
  char *temp_buf = buf;
  int temp_n = n;

  if (*state == HTTP_AWAITING_HEADERS) {
    char *header_end = NULL;
    *state = handle_http_headers(temp_buf, temp_n, remaining_bytes, is_chunked, &header_end, content_type);

    if (*state == 0)
      return 1;

    if (!header_end) {
      *response_buf = ck_realloc(*response_buf, *len + temp_n);
      memcpy(*response_buf + *len, temp_buf, temp_n);
      *len += temp_n;
      return 1;
    }

    if (*state != HTTP_READING_UNTIL_CONNECTION_IS_CLOSED) {
      *response_buf = ck_realloc(*response_buf, *len + (header_end - buf));
      memcpy(*response_buf + *len, temp_buf, header_end - buf);
      *len += header_end - buf;

      temp_buf = header_end;
      temp_n = n - (header_end - buf);
    }
  }

  if (*state == HTTP_READING_BODY) {
    if (*remaining_bytes > 0) {
      *response_buf = ck_realloc(*response_buf, *len + temp_n);
      memcpy(*response_buf + *len, temp_buf, temp_n);
      *len += temp_n;
      *remaining_bytes -= temp_n;
      return (*remaining_bytes <= 0) ? 0 : 1;
    }
  }

  if (*state == HTTP_READING_CHUNKS) {
    int result = handle_chunked_body(temp_buf, temp_n, remaining_bytes);
    *response_buf = ck_realloc(*response_buf, *len + temp_n);
    memcpy(*response_buf + *len, temp_buf, temp_n);
    *len += temp_n;
  ...
  // if (*state == HTTP_READING_UNTIL_END_TAG) {
  //   int res = handle_html_xml_body(temp_buf, temp_n, response_buf, len, state, *content_type);
  //   return res;
  // }

  if (*state == HTTP_READING_UNTIL_CONNECTION_IS_CLOSED) {
    *response_buf = ck_realloc(*response_buf, *len + n);
    memcpy(*response_buf + *len, buf, n);
    *len += n;
  }
...
int handle_raw_protocol(char *buf, int n, char **response_buf, unsigned int *len, int *state, int *remaining_bytes, int *is_chunked, char **content_type) {
  *response_buf = ck_realloc(*response_buf, *len + n);
...
int handle_html_xml_body(char *buf, int n, char **response_buf, unsigned int *len, int *state, char *content_type) {
  static int last_content_type_val = 0;
  static char xml_root_tag[64] = {0};
  static int xml_root_tag_len = 0;

  *response_buf = ck_realloc(*response_buf, *len + n);
  memcpy(*response_buf + *len, buf, n);
  *len += n;

  if (strstr(content_type, "text/html") || last_content_type_val == 1) {
    last_content_type_val = 1;
    if (memmem(*response_buf, *len, "</html>", 7)) {
      *state = HTTP_COMPLETE;
      last_content_type_val = 0;
      return 0;
    }
  }
  else if (strstr(content_type, "xml") || last_content_type_val == 2) {
    last_content_type_val = 2;
    if (xml_root_tag_len == 0) {
      char *start = memmem(*response_buf, *len, "<", 1);
      char *end = memmem(*response_buf, *len, ">", 1);
      if (start && end && (end - start) < 63) {
        memcpy(xml_root_tag, start + 1, end - start - 1);
        xml_root_tag[ end - start - 1 ] = '\0';
        xml_root_tag_len = strlen(xml_root_tag);
      }
    }
    
    if (xml_root_tag_len > 0) {
      char closing_tag[72];
      snprintf(closing_tag, sizeof(closing_tag), "</%s>", xml_root_tag);
      if (memmem(*response_buf, *len, closing_tag, strlen(closing_tag))) {
        *state = HTTP_COMPLETE;
        last_content_type_val = 0;
        return 0;
      }
...
int handle_http_headers(char *buf, int n, int *remaining_bytes, int *is_chunked, char **header_end, char **content_type) {
  *header_end = memmem(buf, n, HTTP_TERMINATOR, strlen(HTTP_TERMINATOR));
  if (*header_end == 0) {
    *header_end = memmem(buf, n, HTTP_TERMINATOR2, strlen(HTTP_TERMINATOR2));
    if (*header_end == 0)
      return HTTP_AWAITING_HEADERS;
    else {
      *header_end += strlen(HTTP_TERMINATOR2);
    }
  }
  else {
    *header_end += strlen(HTTP_TERMINATOR);
  }

  char *content_length_pos = memmem(buf, n, "Content-Length:", strlen("Content-Length:"));
  if (content_length_pos) {
    *remaining_bytes = atoi(content_length_pos + strlen("Content-Length:"));
    *is_chunked = 0;
    return HTTP_READING_BODY;
  }

  if (memmem(buf, n, "Transfer-Encoding: chunked", strlen("Transfer-Encoding: chunked"))) {
    *remaining_bytes = 0;
    *is_chunked = 1;
    return HTTP_READING_CHUNKS;
  }

  // char *content_type_pos = memmem(buf, n, "Content-type: ", strlen("Content-type: "));
  // if (content_type_pos) {
  //   content_type_pos += strlen("Content-Type: ");
  //   if (memmem(content_type_pos, n - (content_type_pos - buf), "text/html", strlen("text/html")) ||
  //       memmem(content_type_pos, n - (content_type_pos - buf), "text/xml", strlen("text/xml"))) {
  //     *content_type = content_type_pos;
  //     return HTTP_READING_UNTIL_END_TAG;
  //   }
  // }

  return HTTP_READING_UNTIL_CONNECTION_IS_CLOSED;
}

int handle_chunked_body(char *buf, int n, int *remaining_bytes) {
  char *chunk_start = NULL;

  if (*remaining_bytes == 0) {
    char *size_end = memmem(buf, n, CHUNK_TERMINATOR, strlen(CHUNK_TERMINATOR));
    if (!size_end) return 1;

    int chunk_size = strtol(buf, NULL, 16);
    if (chunk_size == 0) {
      chunk_size = 0;
      return 0;
    }
    ...
    *remaining_bytes -= n-(chunk_start-buf)-strlen(CHUNK_TERMINATOR);
  }
  else {
    *remaining_bytes -= n;
  }

  char *http_terminator = memmem(buf+n-strlen(HTTP_CHUNKED_TERMINATOR), strlen(HTTP_CHUNKED_TERMINATOR), HTTP_CHUNKED_TERMINATOR, strlen(HTTP_CHUNKED_TERMINATOR));
  if (http_terminator) return 0;
...
region_t* extract_requests_tftp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
...
region_t* extract_requests_dhcp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
...
region_t* extract_requests_SNTP(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
...
region_t* extract_requests_NTP(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
...
region_t* extract_requests_SNMP(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
...
region_t* extract_requests_smtp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
   char *mem;
...
region_t* extract_requests_ssh(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
region_t* extract_requests_tls(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
region_t* extract_requests_dicom(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
...
region_t* extract_requests_dns(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
...
region_t* extract_requests_rtsp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
region_t* extract_requests_ftp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
region_t* extract_requests_sip(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
region_t* extract_requests_http(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
  ...
  unsigned int region_count = 0;
  region_t *regions = NULL;
  // char terminator[6] = {0x0D, 0x0D, 0x0A, 0x0D, 0x0D, 0x0A};
  ...
  while (byte_count < buf_size) {
    ...
    //Check if the last six bytes are the region delimiter
    if ((mem_count >=strlen(region_delimiter)) && (memcmp(&mem[mem_count - (strlen(region_delimiter)-1)], region_delimiter, strlen(region_delimiter)) == 0)) {
      region_count++;
      regions = (region_t *)ck_realloc(regions, region_count * sizeof(region_t));
      regions[region_count - 1].start_byte = cur_start;
      regions[region_count - 1].end_byte = cur_end;
...
region_t* extract_requests_ipp(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_tftp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_dhcp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_SNTP(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_NTP(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_SNMP(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_smtp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_ssh(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
   char mem[7];
   unsigned int byte_count = 0;
   unsigned int *state_sequence = NULL;
...
unsigned int* extract_response_codes_tls(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_dicom(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  if (buf_size == 0) {
    *state_count_ref = 0;
    return NULL;
...
unsigned int* extract_response_codes_dns(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int mem_count = 0;
  unsigned int mem_size = 1024;
...
region_t *extract_requests_dtls12(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter) {
  unsigned int byte_count = 0;
  unsigned int region_count = 0;
  region_t *regions = NULL;
...
//
unsigned int* extract_response_codes_dtls12(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  unsigned int byte_count = 0;
  unsigned int *state_sequence = NULL;
  unsigned int state_count = 0;
...
unsigned int* extract_response_codes_rtsp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_ftp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_sip(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
unsigned int* extract_response_codes_http(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  volatile unsigned int byte_count = 0;
  volatile unsigned int mem_count = 0;
  volatile unsigned int mem_size = 1024;
  unsigned int *state_sequence = NULL;
  unsigned int state_count = 0;
  ...
  while (byte_count < buf_size) {
    char c = buf[byte_count++];

    memcpy(&mem[mem_count], &c, 1);

    if (mem_count >= 5 && memcmp(mem, http, 5) == 0) {
      if (byte_count + 2 < buf_size) {
        char temp[4] = {0};
        memcpy(temp, &buf[byte_count + 3], 3);
        unsigned int message_code = (unsigned int)atoi(temp);
...
unsigned int* extract_response_codes_ipp(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug)
{
  char *mem;
  unsigned int byte_count = 0;
  unsigned int mem_count = 0;
...
void delete_kl_messages_from(klist_t(lms) *kl_messages, kliter_t(lms) *start)
{
  message_t *m;

  int ret = kl_shift_from(lms, kl_messages, start, &m);
  ...
  while (ret == 0) {
    ...
    ret = kl_shift_from(lms, kl_messages, start, &m);
  }
}
...
u32 save_kl_messages_to_file(klist_t(lms) *kl_messages, u8 *fname, u8 replay_enabled, u32 max_count, int *regions_to_keep, int debug)
{
  u8 *mem = NULL;
  u32 len = 0, message_size = 0;
  kliter_t(lms) *it;

  s32 fd = open(fname, O_WRONLY | O_CREAT, 0777);
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages) && message_count < max_count; it = kl_next(it)) {
    if (regions_to_keep && !regions_to_keep[message_count]) {
      if (debug) {
        ...
        fprintf(fp, "\tsave_kl_messages_to_file: discarded %d\n", message_count);
        fclose(fp);
      }
...
int net_recv(int sockfd, struct timeval timeout, char **response_buf, unsigned int *len, char protocol) {
  if (sockfd < 0) {
    fprintf(stderr, "Invalid socket fd in net_recv.\n");
    return 1;
  }

  char temp_buf[TEMP_BUF_SIZE];
  int state = HTTP_AWAITING_HEADERS;
  protocol_handler_t protocol_handler;

  switch (protocol) {
    case HTTP:
      protocol_handler = handle_http_protocol;
      break;
    default:
      protocol_handler = handle_raw_protocol;
      break;
  }
  ...
  *len = 0;
  ...
  while (1) {
    ...
    do {
      ret = safe_poll(&pfd, 1, remaining_timeout_ms);
    ...
    if (pfd.revents & POLLIN) {
      ...
      int result = protocol_handler(temp_buf, n, response_buf, len, &state, &remaining_bytes, &is_chunked, &content_type);
      if (result == 0) return 0;
    }
  }
...
void save_regions_to_file(region_t *regions, unsigned int region_count, unsigned char *fname)
{
  ...
  fd = open(fname, O_WRONLY | O_CREAT | O_EXCL, 0777);
