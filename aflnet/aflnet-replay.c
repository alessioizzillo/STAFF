...
int main(int argc, char* argv[])
{
  ...
  while(!feof(fp)) {
    ...
    if (fread(&size, sizeof(unsigned int), 1, fp) > 0) {
      ...
      if (net_recv(sockfd, timeout, &response_buf, &response_buf_size, protocol)) break;
  ...
  state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count, 0);
