...
u8* extract_filename(u8* path) {
  const u8* fn = path + strlen((const char*)path);
  while (fn > path && fn[-1] != '/') fn--;
  return (u8*)fn;
}

u8* replace_dir(u8* orig_path, u8* new_dir) {
  u8* filename = extract_filename(orig_path);
  return alloc_printf("%s/%s", new_dir, filename);
}

int move_file(const char *source_path, const char *dest_dir) {
  ...
  u8 *dest_path = alloc_printf("%s/%s", dest_dir, filename);

  if (rename(source_path, (char *)dest_path) != 0) {
    PFATAL("Failed to move file from '%s' to '%s'", source_path, dest_path);
  }
...
EXP_ST u32 fuzz_tmout = 0; /* Configurable fuzz timeout (s)   */
...
static int calibration;               /* Try to calibrate faster?         */
...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           queue_cycle = 0,               /* Queue round counter              */
...
struct queue_entry {
  ...
  u8  taint_analyzed,                 /* Taint analyzed?                  */
...
static struct queue_entry *queue,     /* Fuzzing queue (linked list)      */
                          ...
                          *old_taint_analyzed_queue_cur = NULL,
                          *old_queue_cur = NULL,
...
u32 server_wait_secs = 120;
u32 poll_wait_msecs = 1;
u32 child_tmout_secs = 1;
u32 max_tmout_secs = 1;
u8 net_protocol;
u8 protocol;
u8* net_ip;
u32 net_port;
...
u32 state_cycles = 0;
u32 messages_sent = 0, num_messages = 0;
...
u8  show_stats_tmp[256];

static void visualize_stats(void);
...
unsigned int* (*extract_response_codes)(unsigned char* buf, unsigned int buf_size, unsigned int* state_count_ref, int debug) = NULL;
region_t* (*extract_requests)(unsigned char* buf, unsigned int buf_size, unsigned int* region_count_ref, char *region_delimiter) = NULL;

void update_taint_fetch_count(const char* fn, const char* out_dir, int fetch_count) {
  char taint_dir[4096];
  snprintf(taint_dir, sizeof(taint_dir), "%s/taint_queue_fetch", out_dir);

  if (access(taint_dir, F_OK) != 0) {
    if (mkdir(taint_dir, 0755) != 0 && errno != EEXIST) {
  ...
  char* base_fn = basename(fn_copy);

  char taint_fn[4096];
  snprintf(taint_fn, sizeof(taint_fn), "%s/%s", taint_dir, base_fn);

  FILE* fp = fopen(taint_fn, "w");
  if (fp) {
    fprintf(fp, "%d\n", fetch_count);
    fclose(fp);
  } else {
...
void create_directory(const char *path) {
  ...
  if (stat(path, &st) == 0 && S_ISDIR(st.st_mode)) {
    printf("Directory exists. Removing...\n");
...
u64 extract_u64_after_dollar(u8 *str) {
  u8 *dollar = strchr(str, '$');
  if (!dollar || *(dollar + 1) == '\0') {
    fprintf(stderr, "No dollar sign followed by digits found in input.\n");
    return 0;
  }

  u8 *endptr;
  u64 value = strtoull(dollar + 1, &endptr, 10);

  if (dollar + 1 == endptr) {
    fprintf(stderr, "No valid number after dollar sign.\n");
    return 0;
  }
...
int is_like_with_middle_modification(
  char *buf1, int len1,
  char *buf2, int len2,
  const char *stage_name, int stage_cur, int rand, int t_len
) {
  ...
  if (first_call) {
    if (debug) {
      debug_file = fopen("debug/is_like_with_middle_modification.log", "w");
      if (!debug_file) {
  ...
  if (debug) {
    debug_file = fopen("debug/is_like_with_middle_modification.log", "a");
    if (!debug_file) {
      perror("Error opening debug file");
      return -1;
    }

    fprintf(debug_file, "Stage: %s, Iteration: %d, Rand: %d\n", stage_name, stage_cur, rand);

    fprintf(debug_file, "buf1 length: %d\n", len1);
    fprintf(debug_file, "buf2 length: %d\n", len2);
    fprintf(debug_file, "target_offset: %d\n", target_offset);
    fprintf(debug_file, "t_len: %d\n", t_len);
  }

  if (target_offset < 0 || t_len < 0 || target_offset + t_len > len2) {
    if (debug) {
      fprintf(debug_file, "Invalid target_offset or t_len.\n");
      fclose(debug_file);
    }
    return 0;
  }

  int prefix_len = target_offset;
  int suffix_len = len2 - (target_offset + t_len);

  if (debug) {
    fprintf(debug_file, "Prefix length: %d\n", prefix_len);
    fprintf(debug_file, "Suffix length: %d\n", suffix_len);

    fprintf(debug_file, "buf1 (char): ");

    for (int i = 0; i < len1; i++) fprintf(debug_file, "%c", buf1[i]);
    fprintf(debug_file, "\nbuf1 (hex): ");
    for (int i = 0; i < len1; i++) fprintf(debug_file, "%02X ", (unsigned char)buf1[i]);
    fprintf(debug_file, "\n");

    fprintf(debug_file, "buf2 (char): ");
    for (int i = 0; i < len2; i++) fprintf(debug_file, "%c", buf2[i]);
    fprintf(debug_file, "\nbuf2 (hex): ");
    for (int i = 0; i < len2; i++) fprintf(debug_file, "%02X ", (unsigned char)buf2[i]);
    fprintf(debug_file, "\n");
  }

  if (prefix_len > 0) {
    if (len1 < prefix_len || memcmp(buf1, buf2, prefix_len) != 0) {
      if (debug) {
        fprintf(debug_file, "Prefix mismatch.\n");
        fclose(debug_file);
      }
      return 0;
    }
    if (debug) {
      fprintf(debug_file, "Prefix matches.\n");
    }
  }

  if (suffix_len > 0) {
    if (len1 < suffix_len || memcmp(buf1 + len1 - suffix_len, buf2 + len2 - suffix_len, suffix_len) != 0) {
      if (debug) {
        fprintf(debug_file, "Suffix mismatch.\n");
        fclose(debug_file);
      }
      return 0;
    }
    if (debug) {
      fprintf(debug_file, "Suffix matches.\n");
    }
  }

  if (debug) {
    fprintf(debug_file, "Buffers match with middle modification.\n");
    fprintf(debug_file, "______________________________________________________________\n");
    fclose(debug_file);
  }
  return 1;
}
  ...
  for (i = 0; i < messages_sent; i++) {
    if ((response_bytes[i] == 0) || ( i > 0 && (response_bytes[i] - response_bytes[i - 1] == 0))) {
      ...
      if (debug) {
        ...
        fprintf(fp, "\tupdate_region_annotations(): response_bytes[%d] = %d, messages_sent = %d\n", i, response_bytes[i], messages_sent);
        fclose(fp);
      }
      
      q->regions[i].state_sequence = (*extract_response_codes)(response_buf, response_bytes[i], &state_count, debug);
...
void update_fuzzs() {
  ...
  unsigned int *state_sequence = (*extract_response_codes)(response_buf, response_buf_size, &state_count, debug);
...
static void load_ipsm_from_file() {
  char *filename = alloc_printf("%s/ipsm.dot", out_dir);

  OKF("Opening IPSM file: %s", filename);

  ipsm_dot_file = fopen(filename, "r");
  if (!ipsm_dot_file) {
    FATAL("Cannot open %s", filename);
  }

  ipsm = agread(ipsm_dot_file, NULL);
  if (!ipsm) {
    FATAL("Cannot read graph from %s", filename);
  }

  Agnode_t *n;
  for (n = agfstnode(ipsm); n; n = agnxtnode(ipsm, n)) {
    char *name = agnameof(n);
    OKF("Processing node: %s", name);
    unsigned int state_id = atoi(name);

    state_info_t *new_state = (state_info_t *)ck_alloc(sizeof(state_info_t));
    new_state->id = state_id;
    new_state->is_covered = 1;
    new_state->paths = 0;
    ...
    int ret;
    khiter_t k = kh_put(hms, khms_states, state_id, &ret);
    if (!ret) {
      FATAL("Duplicate state name %d in IPSM", state_id);
    }
    kh_value(khms_states, k) = new_state;

    state_ids = (u32 *) ck_realloc(state_ids, (state_ids_count + 1) * sizeof(u32));
    state_ids[state_ids_count++] = state_id;

    u8 *state_file_path = alloc_printf("%s/states/%u", out_dir, state_id);
    OKF("Looking for state file: %s", state_file_path);

    FILE *f = fopen((const char *)state_file_path, "r");
    if (f) {
      OKF("Found state file: %s", state_file_path);
      ...
      while (fgets(buf, sizeof(buf), f)) {
        ...
        OKF("Processing seed file: %s", buf);

        struct queue_entry *q_it;
        for (q_it = queue; q_it; q_it = q_it->next) {
          if (strcmp((const char *)q_it->fname, (const char *)buf) == 0) {
            OKF("Found matching seed: %s", q_it->fname);
            new_state->seeds = (void **) ck_realloc(new_state->seeds, (new_state->seeds_count + 1) * sizeof(void *));
            new_state->seeds[new_state->seeds_count] = (void *)q_it;
            new_state->seeds_count++;
            break;
          }
        }
      }
      ...
      OKF("State file not found: %s", state_file_path);
    }

    ck_free(state_file_path);
  }

  OKF("Finished loading IPSM from file: %s", filename);
...
void update_state_aware_variables(struct queue_entry *q, u8 dry_run)
{
  ...
  if (is_state_sequence_interesting(state_sequence, state_count)) {
    ...
    // // if (taint_aware_mode && !regions_to_keep)
    // //   PFATAL("regions_to_keep is NULL");
    // save_kl_messages_to_file(kl_messages, fname, 1, messages_sent, regions_to_keep, debug);
    // ck_free(temp_str);
    // ck_free(fname);
  ...
  if (k != kh_end(khms_states)) {
    ...
    // Append fname to the file out_dir/0
    u8 *state_file_path = alloc_printf("%s/states/0", out_dir);
    FILE *f = fopen((const char *)state_file_path, "a+");
    if (!f) PFATAL("Cannot open state seeds file for appending");
    fprintf(f, "%s\n", q->fname);
    fclose(f);
    ck_free(state_file_path);
  ...
  for (i = 0; i < q->region_count; i++) {
    ...
    if (regional_state_count > 0) {
      ...
      if (k != kh_end(khms_states)) {
        ...
        if (debug) {
          FILE *fp = fopen("debug/fuzzing.log", "a+");
          fprintf(fp, "\tupdate_state_aware_variables(): state->seeds_count %d\n", state->seeds_count);
          fclose(fp);
        }
      ...
      u8 *state_file_path = alloc_printf("%s/states/%u", out_dir, reachable_state_id);
      FILE *f = fopen((const char *)state_file_path, "a+");
      if (!f) PFATAL("Cannot open state seeds file for appending");
      fprintf(f, "%s\n", q->fname);
      fclose(f);
      ...
      was_fuzzed_map[get_state_index(reachable_state_id)][q->index] = 0; // Mark as reachable but not fuzzed
    }
  }
...
int send_over_network(int checkpoint)
{
  int n, alarm = 0;
  u8 likely_buggy = 0;
  static struct sockaddr_in serv_addr;
  static struct sockaddr_in local_serv_addr;
  ...
  if (debug) {
    ...
    fprintf(fp, "\tsend_over_network (checkpoint: %d): === Starting send_over_network ===\n", checkpoint);
    fclose(fp);
  }
  ...
  if (debug) {
    fp = fopen("debug/fuzzing.log", "a+");
    fprintf(fp, "\tsend_over_network (checkpoint: %d): connected to the server!\n", checkpoint);
    fclose(fp);
  }

  // //retrieve early server response if needed
  // if (net_recv(sockfd, &timeout, &response_buf, &response_buf_size)) goto HANDLE_RESPONSES;

  //write the request messages
  kliter_t(lms) *it;
  messages_sent = 0;

  klist_t(lms) *kl_m = kl_messages;

  int fail = 0, retry = 0;
  struct timeval check_0, check_1, check_2, check_3;
  ...
  for (it = kl_begin(kl_m); it != kl_end(kl_m); it = kl_next(it), messages_sent++) {
    ...
    timeout.tv_sec = dry_run_phase ? child_tmout_secs : max_tmout_secs;
    timeout.tv_usec = 0;

retry:
    if (debug) gettimeofday(&check_0, NULL);

    if (debug) {
      fp = fopen("debug/fuzzing.log", "a+");
      fprintf(fp, "\tsend_over_network (checkpoint: %d, timeout: %u): sending region %d\n", checkpoint, timeout.tv_sec, messages_sent);
      fclose(fp);
    }

    size_t total_sent = 0;
    size_t bytes_to_send = kl_val(it)->msize - strlen(region_delimiter);
    while (total_sent < bytes_to_send) {
        ssize_t n = net_send(sockfd, timeout, kl_val(it)->mdata + total_sent, bytes_to_send - total_sent);
        if (n == -1) {

            if (debug) {
              fp = fopen("debug/fuzzing.log", "a+");
              fprintf(fp, "\tsend_over_network (checkpoint: %d): Error during network send (messages_sent %d, errno = %d)", checkpoint, messages_sent, errno);
              fclose(fp);
            }
            
            WARNF("Error during network send (errno = %d)", errno);
            break;
        }
        total_sent += n;
    }
    ...
    if (debug) {
      fp = fopen("debug/fuzzing.log", "a+");
      fprintf(fp, "\tsend_over_network (delimiter: %d) (checkpoint: %d): region %d sent %.3f ms, total_sent = %zu of %zu bytes, errno = %d\n", 
        strlen(region_delimiter), checkpoint, messages_sent, 
        ((check_1.tv_sec - check_0.tv_sec) * 1000.0) + ((check_1.tv_usec - check_0.tv_usec) / 1000.0), 
        total_sent, bytes_to_send, errno);
      fclose(fp);
    }

    response_bytes = (u32 *) ck_realloc(response_bytes, (messages_sent+1) * sizeof(u32));

    if (total_sent != bytes_to_send) {
        WARNF("Incomplete message sent (total_sent = %zu, expected = %zu)", total_sent, bytes_to_send);
        messages_sent++;
        break;
    }

    gettimeofday(&check_2, NULL);
    int res = net_recv(sockfd, timeout, &response_buf, &response_buf_size, protocol);
    gettimeofday(&check_3, NULL);

    if (dry_run_phase) {
      u32 res_time = ((check_3.tv_sec - check_2.tv_sec)) + ((check_3.tv_usec - check_2.tv_usec) / 1000000) + 1;
      max_tmout_secs = max_tmout_secs <= res_time ? res_time : max_tmout_secs;
    }

    if (debug) {
      fp = fopen("debug/fuzzing.log", "a+");
      fprintf(fp, "\tsend_over_network (checkpoint: %d): response of the region %d received in %.3f ms (%d response_size, res = %d, errno = %d)\n", 
        checkpoint, messages_sent, 
        ((check_3.tv_sec - check_0.tv_sec) * 1000.0) + ((check_3.tv_usec - check_0.tv_usec) / 1000.0), 
        response_buf_size, res, errno);
      fclose(fp);
    }
    ...
    if (res) {
      if (res == 1 || res == 3) {
        ...
        if (debug) {
          fp = fopen("debug/fuzzing.log", "a+");
          fprintf(fp, "\tsend_over_network (checkpoint: %d): connection has gone down, reconnecting...!\n", checkpoint, messages_sent);
          fclose(fp);
        }
        ...
        if (res == 3 && !retry) {
          ...
          if (debug) {
            fp = fopen("debug/fuzzing.log", "a+");
            fprintf(fp, "\tsend_over_network (checkpoint: %d): retry...\n", checkpoint);
            fclose(fp);
          }
        ...
        if (debug) {
          fp = fopen("debug/fuzzing.log", "a+");
          fprintf(fp, "\tsend_over_network (checkpoint: %d): timeout in 'net_recv()' for region %d!\n", checkpoint, messages_sent);
          fclose(fp);
        }
...
send_signals:
  ...
  if (debug) {
    fp = fopen("debug/fuzzing.log", "a+");
    fprintf(fp, "\tsend_over_network (checkpoint: %d): closed connection! (messages_sent = %d)\n", checkpoint, messages_sent);
    fclose(fp);
  }
  ...
  if (checkpoint) {
    ...
    *checkpoint_forksrv = 1;
    ...
    if (alarm) {
      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tsend_over_network (checkpoint: %d): sending SIGALRM to %d...\n", checkpoint, child_pid);
        fclose(fp);
      }
      if (child_pid > 0) kill(child_pid, SIGALRM);
      ...
      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tsend_over_network (checkpoint: %d): === Ending send_over_network ===\n", checkpoint);
        fclose(fp);
      }
      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tsend_over_network (checkpoint: %d): sending SIGINT to %d...\n", checkpoint, child_pid);
        fclose(fp);
      }
...
static void add_to_queue(u8* fname, u8* fname_trace, u32 len, u8 passed_det, u8 taint_analyzed, u8 from_crashes, u8 from_hangs) {

  struct queue_entry* q = ck_alloc(sizeof(struct queue_entry));

  q->fname        = fname;
  q->fname_trace  = fname_trace;
  q->from_crashes = from_crashes;
  ...
  if (corpus_read_or_sync) {
    ...
    q->regions = (*extract_requests)(buf, len, &q->region_count, region_delimiter);
    ck_free(buf);
...
EXP_ST void write_bitmap(void) {
  ...
  fname = alloc_printf("%s/fuzz_bitmap_eval", out_dir);
  fd = open(fname, O_WRONLY | O_CREAT | O_TRUNC, 0777);
...
EXP_ST void read_bitmap(u8* fname) {
  ...
  u8* fname2 = alloc_printf("%s_eval", fname);
...
static u32 count_non_255_bytes_excluding_var(u8* mem) {
...
static const u8 count_class_lookup8_eval[256] = {
...
static u16 count_class_lookup16_eval[65536];
...
EXP_ST void init_count_class16_eval(void) {

  u32 b1, b2;

  for (b1 = 0; b1 < 256; b1++) 
    for (b2 = 0; b2 < 256; b2++)
      count_class_lookup16_eval[(b1 << 8) + b2] = 
        (count_class_lookup8_eval[b1] << 8) |
        count_class_lookup8_eval[b2];
...
static inline void classify_counts_eval(u64* mem) {
  ...
  while (i--) {
    ...
    if (unlikely(*mem)) {
      ...
      mem16[0] = count_class_lookup16_eval[mem16[0]];
      mem16[1] = count_class_lookup16_eval[mem16[1]];
      mem16[2] = count_class_lookup16_eval[mem16[2]];
      mem16[3] = count_class_lookup16_eval[mem16[3]];
...
static void remove_shm(void) {
  ...
  shmctl(shm_id_eval, IPC_RMID, NULL);
...
EXP_ST void setup_shm(void) {
  ...
  shm_str = alloc_printf("%d", shm_id_eval);
  ...
  if (!dumb_mode) setenv(SHM_ENV_VAR_EVAL, shm_str, 1);

  ck_free(shm_str);

  trace_bits_eval = shmat(shm_id_eval, NULL, 0);
  
  if (trace_bits_eval == (void *)-1) PFATAL("shmat() failed");
...
static void write_resume_time(void) {

  u8 *fn = alloc_printf("%s/resume_ts", out_dir);
  FILE *f = fopen(fn, "a+");
  ck_free(fn);
...
static void write_max_tmout_secs(void) {

  u8 *fn = alloc_printf("%s/max_tmout_secs", out_dir);
  FILE *f = fopen(fn, "a+");
  ck_free(fn);
...
static void read_max_tmout_secs(void) {
  ...
  if (fscanf(f, "%u", &max_tmout_secs) != 1) {
    fclose(f);
    ck_free(fn);
    return;
  }
...
static void read_testcases(void) {
  ...
  if (replay_mode) {
    crash_dir = alloc_printf("%s/crashes", in_dir);
    crash_traces_dir = alloc_printf("%s/crash_traces", in_dir);
    hang_dir = alloc_printf("%s/hangs", in_dir);
    ...
    if (access(hang_dir, F_OK)) {
      ck_free(hang_dir);
      hang_dir = NULL;
    }

    if (crash_dir) {
      DIR* crash_dir_stream = opendir(crash_dir);
      if (crash_dir_stream) {
        struct dirent* entry;
        while ((entry = readdir(crash_dir_stream)) != NULL) {
          if (entry->d_name[0] == '.') continue;
          u8* crash_fn = alloc_printf("%s/%s", crash_dir, entry->d_name);
          u8* crash_fn_trace = alloc_printf("%s/%s", crash_traces_dir, entry->d_name);
          struct stat st;
          if (lstat(crash_fn, &st) == 0 && access(crash_fn, R_OK) == 0) {
            add_to_queue(crash_fn, crash_fn_trace, st.st_size, 0, 0, 1, 0);
          }
        }
        closedir(crash_dir_stream);
      }
    }

    if (hang_dir) {
      DIR* hang_dir_stream = opendir(hang_dir);
      if (hang_dir_stream) {
        struct dirent* entry;
        while ((entry = readdir(hang_dir_stream)) != NULL) {
          if (entry->d_name[0] == '.') continue;
          u8* hang_fn = alloc_printf("%s/%s", hang_dir, entry->d_name);
          struct stat st;
          if (lstat(hang_fn, &st) == 0 && access(hang_fn, R_OK) == 0) {
            add_to_queue(hang_fn, 0, st.st_size, 0, 0, 0, 1);
          }
        }
        closedir(hang_dir_stream);
      }
    }
  }
  ...
  for (i = 0; i < nl_cnt; i++) {
    ...
    if (taint_aware_mode) {
      initialize_queue(fn, out_dir, "taint_metrics", debug);
      add_alias_app_tb_pc(fn, fn);
      add_alias_cov(fn, fn);
...
EXP_ST void init_forkserver(char** argv) {
  ...
  if (!forksrv_pid) {
    ...
    *(u32*)trace_bits_eval = EXEC_FAIL_SIG;
    exit(0);

  }

  ACTF("Sleeping for %d seconds", server_wait_secs);
  ...
  while (1) {
    ...
    if (!res) {
      ...
      WARNF("Error connect()! %d\n", errno);
    }

    usleep(usleep_time);

    if (usleep_time < MAX_SLEEP) {
      usleep_time *= 2;
      if (usleep_time > MAX_SLEEP) usleep_time = MAX_SLEEP;
    }
  }
  close(sockfd);
  sleep(1);

  ACTF("Setting start_fork_flag to 1");
  *start_fork_flag = 1;
  sleep(1);
  ...
  if (*(u32*)trace_bits == EXEC_FAIL_SIG && *(u32*)trace_bits_eval == EXEC_FAIL_SIG)
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  if (dumb_mode == 1 || no_forkserver) {
    ...
    if (use_net) {
      child_timed_out = send_over_network(0);
    }
    s32 res;
  ...
  if (!WIFSTOPPED(*child_retval)) child_pid = 0;
  ...
  tb4 = *(u32*)trace_bits;
  tb4_eval = *(u32*)trace_bits_eval;

#ifdef WORD_SIZE_64
  classify_counts((u64*)trace_bits);
  classify_counts_eval((u64*)trace_bits_eval);
#else
  ...
  if ((WIFSIGNALED(*child_retval) || child_timed_out) && !stop_soon) {

    kill_signal = WTERMSIG(*child_retval);

    if (kill_signal == 2) return FAULT_TMOUT;
    if (child_timed_out) return FAULT_TMOUT;

    if (kill_signal == SIGINT) return FAULT_NONE;
  ...
  if (uses_asan && WEXITSTATUS(*child_retval) == MSAN_ERROR) {
    kill_signal = 0;
    return FAULT_CRASH;
  }

  /* treat all non-zero return values from qemu system test as a crash */
  if(qemu_mode > 1 && WEXITSTATUS(*child_retval) != 0) {
    return FAULT_CRASH;
  }

  if ((dumb_mode == 1 || no_forkserver) && tb4 == EXEC_FAIL_SIG && tb4_eval == EXEC_FAIL_SIG)
    return FAULT_ERROR;
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  if (calibration == -1)
    ...
    *new_bits = has_new_bits(virgin_bits, trace_bits);
    has_new_bits(virgin_bits_eval, trace_bits_eval);
    ...
    if (calibration == 0) {
      ...
      stage_max  = !dry_run ? 1 : 2;
    else if (calibration == 2)
      stage_max  = !dry_run ? 2 : 3;
    else if (calibration == 3)
      stage_max  = !dry_run ? 3 : 4;
  ...
  for (stage_cur = 0; (stage_cur < stage_max) || auto_calibration; stage_cur++) {
    // OKF("OTHER CYCLE");
    // sleep(15);
    // OKF("GO");
    u32 cksum;
    ...
    if (!dry_run && !dumb_mode && !stage_cur && (!count_bytes(trace_bits) || !count_bytes(trace_bits_eval))) {
      ...
      if (q->exec_cksum) {
        ...
        for (i = 0; i < MAP_SIZE; i++) {
          ...
          if (first_trace_eval[i] != trace_bits_eval[i]) {
            first_trace_eval[i] = first_trace_eval[i] < trace_bits_eval[i] ? first_trace_eval[i] : trace_bits_eval[i];
          }
  ...
  if (hnb > *new_bits) {
    WARNF("New bits discovered: %d", hnb);
    *new_bits = hnb;
  }
...
int is_direct_child(const char* base_dir, const char* file_path) {
  ...
  int result = strcmp(dir_name, base_dir) == 0;
  free(path_copy);
  return result;
}
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    if (state_aware_mode && !enable_taint_aware_mode) update_state_aware_variables(q, 1);

    /* save the seed to file for replaying */
    // u8 *fn_replay = alloc_printf("%s/replayable-queue/%s$%lld", out_dir, basename(q->fname), get_cur_time());
    // save_kl_messages_to_file(kl_messages, fn_replay, 1, messages_sent, NULL, debug);
    // ck_free(fn_replay);
    ...
    switch (res) {
      ...
      case FAULT_TMOUT:
        if (replay_mode) {
          ...
          if (unique_hangs >= KEEP_UNIQUE_HANG) break;
          ...
          if (!dumb_mode) {
            ...
            if (!has_new_bits(virgin_tmout, trace_bits)) break;

            u8* tmp = alloc_printf("%s/hangs", out_dir);
            move_file(q->fname, tmp);
            ck_free(tmp);
          }
      ...
      case FAULT_CRASH:
        if (replay_mode) {
          ...
          if (!dumb_mode) {
            ...
            u8 *tmp = alloc_printf("%s/crash_traces", out_dir); 
...
#  define CASE_PREFIX "id_"
    ...
    add_alias_app_tb_pc(q->fname, nfn);
    add_alias_cov(q->fname, nfn);
    update_taint_fetch_count(nfn, out_dir, 0);
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log", "a+");
      fprintf(fp, "\tsave_if_interesting: messages_sent %d\n", messages_sent);
      fclose(fp);
    }

    // if (taint_aware_mode && !regions_to_keep)
    //   PFATAL("regions_to_keep is NULL");
    u32 full_len = save_kl_messages_to_file(kl_messages, fn, 0, messages_sent, regions_to_keep, debug);

    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log", "a+");
      fprintf(fp, "\tsave_if_interesting: fn %s, full_len %d\n", fn, full_len);
      fclose(fp);
    }

    if (!full_len)
      WARNF("full_len is 0! (1)");
    else {
      /* We use the actual length of all messages (full_len), not the len of the mutated message subsequence (len)*/
      add_to_queue(fn, 0, full_len, 0, 0, 0, 0);
    }

    if (state_aware_mode && !enable_taint_aware_mode) update_state_aware_variables(queue_top, 0);

    /* save the seed to file for replaying */
    // u8 *fn_replay = alloc_printf("%s/replayable-queue/%s$%lld", out_dir, basename(queue_top->fname), get_cur_time()-start_time);
    // if (taint_aware_mode && !regions_to_keep)
    //   PFATAL("regions_to_keep is NULL");
    // save_kl_messages_to_file(kl_messages, fn_replay, 1, messages_sent, regions_to_keep, debug);
    // ck_free(fn_replay);
...
static void find_start_time(void) {
...
static void maybe_update_plot_file(double bitmap_cvg, double eps, u64 ts) {
  ...
  /* Fields in the file:
     ...
     unix_time, paths_total, map_size, unique_crashes, unique_hangs,
     stability, n_calibration */

    fprintf(plot_file,
            "%llu, %u, %0.02f%%, %llu, %llu, %0.02f%%, %llu\n",
            ts / 1000, queued_paths,
            bitmap_cvg, unique_crashes,
            unique_hangs, stab_ratio, total_cal_cycles); /* ignore errors */
  }
  else {
  /* Fields in the file:

     unix_time, cycles_done, execs_done, cur_path, paths_total, pending_total,
     pending_favs, map_size, unique_crashes, unique_hangs, max_depth,
     execs_per_sec, stability, n_fetched_random_hints, n_fetched_state_hints, n_fetched_taint_hints, n_calibration */
...
//   }
  ...
  if (!in_place_resume && !replay_mode) {
    ...
    if (state_aware_mode) {
      fn = alloc_printf("%s/states", out_dir);
      if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
      ck_free(fn);
    }
...
void show_stats(void) {
  ...
  // if (cur_ms - last_plot_ms > PLOT_UPDATE_SEC * 1000) {
    last_plot_ms = cur_ms;
    maybe_update_plot_file(t_byte_ratio, avg_exec, 0);
  ...
  if (debug) {
    FILE *fp = fopen("debug/fuzzing.log", "a+");
    fprintf(fp, "\tshow_stats: fuzz_tmout %d, cur_ms %lld, start_time %lld, cond %d\n", fuzz_tmout, cur_ms, start_time, cur_ms > start_time+(fuzz_tmout*1000));
    fclose(fp);
  }
...
static void visualize_stats(void) {
  ...
  memset(show_stats_tmp, ' ', banner_pad);

  sprintf(show_stats_tmp + banner_pad, "%s " cLCY VERSION cLGN
          " (%s) (timeout: %d secs)",  crash_mode ? cPIN "peruvian were-rabbit" :
          cYEL "american fuzzy lop", use_banner, max_tmout_secs);
...
#define SP20    SP10 SP10
  ...
  if (dumb_mode) {
    ...
    if (queue_cycle == 1 || min_wo_finds < 15) strcpy(show_stats_tmp, cMGN); else

    /* Subsequent cycles, but we're still making finds. */
    if (cycles_wo_finds < 25 || min_wo_finds < 30) strcpy(show_stats_tmp, cYEL); else

    /* No finds for a long time and no test cases to try. */
    if (cycles_wo_finds > 100 && !pending_not_fuzzed && min_wo_finds > 120)
      strcpy(show_stats_tmp, cLGN);

    /* Default: cautiously OK to stop? */
    else strcpy(show_stats_tmp, cLBL);

  }

  SAYF(bV bSTOP "        run time : " cRST "%-34s " bSTG bV bSTOP
       "  cycles done : %s%-5s  " bSTG bV "\n",
       DTD(cur_ms, start_time), show_stats_tmp, DI(queue_cycle - 1));
  ...
  sprintf(show_stats_tmp, "%s%s", DI(unique_crashes),
  ...
  sprintf(show_stats_tmp, "%s%s", DI(unique_hangs),
         (unique_hangs >= KEEP_UNIQUE_HANG) ? "+" : "");

  SAYF(bV bSTOP "  last uniq hang : " cRST "%-34s " bSTG bV bSTOP
       "   uniq hangs : " cRST "%-6s " bSTG bV "\n",
       DTD(cur_ms, last_hang_time), show_stats_tmp);
  ...
  sprintf(show_stats_tmp, "%s (%0.02f%%)", DI(cur_skipped_paths),
          ((double)cur_skipped_paths * 100) / queued_paths);

  SAYF(bV bSTOP " paths timed out : " cRST "%-17s " bSTG bV, show_stats_tmp);

  sprintf(show_stats_tmp, "%0.02f bits/tuple",
  ...
  sprintf(show_stats_tmp, "%s (%0.02f%%)", DI(queued_favored),
          ((double)queued_favored) * 100 / queued_paths);

  /* Yeah... it's still going on... halp? */

  SAYF(bV bSTOP "  now trying : " cRST "%-21s " bSTG bV bSTOP
       " favored paths : " cRST "%-22s " bSTG bV "\n", stage_name, show_stats_tmp);

  if (!stage_max) {

    sprintf(show_stats_tmp, "%s/-", DI(stage_cur));

  } else {

    sprintf(show_stats_tmp, "%s/%s (%0.02f%%)", DI(stage_cur), DI(stage_max),
            ((double)stage_cur) * 100 / stage_max);

  }

  SAYF(bV bSTOP " stage execs : " cRST "%-21s " bSTG bV bSTOP, show_stats_tmp);

  sprintf(show_stats_tmp, "%s (%0.02f%%)", DI(queued_with_cov),
          ((double)queued_with_cov) * 100 / queued_paths);

  SAYF("  new edges on : " cRST "%-22s " bSTG bV "\n", show_stats_tmp);

  sprintf(show_stats_tmp, "%s (%s%s unique)", DI(total_crashes), DI(unique_crashes),
          (unique_crashes >= KEEP_UNIQUE_CRASH) ? "+" : "");
  ...
  if (crash_mode) {
    ...
    SAYF(bV bSTOP " total execs : " cRST "%-21s " bSTG bV bSTOP
         ...
         unique_crashes ? cLRD : cRST, show_stats_tmp);

  } else {

    SAYF(bV bSTOP " total execs : " cRST "%-10s (%-4s/%-4s)" bSTG bV bSTOP
         " total crashes : %s%-22s " bSTG bV "\n", DI(total_execs), DI(messages_sent), DI(num_messages),
         unique_crashes ? cLRD : cRST, show_stats_tmp);
  ...
  if (avg_exec < 100) {
    ...
    sprintf(show_stats_tmp, "%s/sec (%s)", DF(avg_exec), avg_exec < 20 ?
            "zzzz..." : "slow!");

    SAYF(bV bSTOP "  exec speed : " cLRD "%-21s ", show_stats_tmp);

  } else {

    sprintf(show_stats_tmp, "%s/sec", DF(avg_exec));
    SAYF(bV bSTOP "  exec speed : " cRST "%-21s ", show_stats_tmp);

  }

  sprintf(show_stats_tmp, "%s (%s%s unique)", DI(total_tmouts), DI(unique_tmouts),
          (unique_hangs >= KEEP_UNIQUE_HANG) ? "+" : "");
  ...
  if (skip_deterministic) {
    ...
    strcpy(show_stats_tmp, "n/a, n/a, n/a");

  } else {

    sprintf(show_stats_tmp, "%s/%s, %s/%s, %s/%s",
  ...
  SAYF(bV bSTOP "  byte flips : " cRST "%-37s " bSTG bV bSTOP "   pending : "
       cRST "%-10s " bSTG bV "\n", show_stats_tmp, DI(pending_not_fuzzed));
  ...
  SAYF(bV bSTOP " arithmetics : " cRST "%-37s " bSTG bV bSTOP "  pend fav : "
       cRST "%-10s " bSTG bV "\n", show_stats_tmp, DI(pending_favored));
  ...
  SAYF(bV bSTOP "  known ints : " cRST "%-37s " bSTG bV bSTOP " own finds : "
       cRST "%-10s " bSTG bV "\n", show_stats_tmp, DI(queued_discovered));
  ...
  sprintf(show_stats_tmp, "%s/%s, %s/%s",
          DI(stage_finds[STAGE_HAVOC]), DI(stage_cycles[STAGE_HAVOC]),
          DI(stage_finds[STAGE_SPLICE]), DI(stage_cycles[STAGE_SPLICE]));

  SAYF(bV bSTOP "       havoc : " cRST "%-37s " bSTG bV bSTOP, show_stats_tmp);

  if (t_bytes) sprintf(show_stats_tmp, "%0.02f%%", stab_ratio);
    else strcpy(show_stats_tmp, "n/a");

  SAYF(" stability : %s%-10s " bSTG bV "\n", (stab_ratio < 85 && var_byte_count > 40)
       ? cLRD : ((queued_variable && (!persistent_mode || var_byte_count > 20))
       ? cMGN : cRST), show_stats_tmp);

  if (!bytes_trim_out) {

    sprintf(show_stats_tmp, "n/a, ");

  } else {

    sprintf(show_stats_tmp, "%0.02f%%/%s, ",
  ...
  SAYF(bV bSTOP "        trim : " cRST "%-37s " bSTG bVR bH20 bH2 bH2 bRB "\n"
       bLB bH30 bH20 bH2 bH bRB bSTOP cRST RESET_G1, show_stats_tmp);
...
EXP_ST u8 common_fuzz_stuff(char** argv, u8* out_buf, u32 len) {
  ...
  u32 region_count = 0;
  region_t *regions = (*extract_requests)(out_buf, len, &region_count, region_delimiter);
...
static u32 choose_block_len(u32 limit) {
  ...
  u32 rlim = queue_cycle ? MIN(queue_cycle, 3) : 1;
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  if (!total_cal_cycles || !total_bitmap_entries)
    return perf_score;
...
static u8 fuzz_one(char** argv) {
  if (debug) {
    FILE *fp = fopen("debug/fuzzing.log", "a+");
    fprintf(fp, "\tfuzz_one (fname: %s): starting...\n", queue_cur->fname);
    fclose(fp);
  }
  s32 len, fd, temp_len, temp_target_len, i, j;
  ...
  if (debug) {
    FILE *fp = fopen("debug/fuzzing.log", "a+");
    fprintf(fp, "\tfuzz_one (fname: %s): pending favored are %d\n", queue_cur->fname, pending_favored);
    fclose(fp);
  }  
...
AFLNET_REGIONS_SELECTION:;
  ...
  if (debug) {
    FILE *fp = fopen("debug/fuzzing.log", "a+");
    fprintf(fp, "\tfuzz_one (fname: %s): CHOSEN!\n", queue_cur->fname);
    fclose(fp);
  } 
  ...
  if (state_aware_mode && !enable_taint_aware_mode) {
    ...
    target_region = M2_start_region_ID;
    target_offset = 0;
    target_len = -1;
    regions_to_keep = NULL;

    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log", "a+");
      fprintf(fp, "\tfuzz_one [STATE_AWARE_MODE] (fname: %s, is_initial_seed: %d): target_region %d, target_offset %d, target_len %d\n", queue_cur->fname, queue_cur->is_initial_seed, target_region, target_offset, target_len);
      fclose(fp);
    }
    ...
    if (enable_taint_aware_mode && taint_aware_mode && queue_cur->is_initial_seed && !no_more_taint_hints) {
      ...
      target_len = qe.length;
      M2_start_region_ID = qe.region_num;

      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tfuzz_one (fname: %s): M2_start_region_ID = qe.region_num (%d)\n", queue_cur->fname, M2_start_region_ID);
        fclose(fp);
      }

      target_region = M2_start_region_ID;
      M2_region_count = 1;
      target_offset = qe.offset;
      if (regions_to_keep != NULL)
        free(regions_to_keep);
      if (sequence_minimization)
        regions_to_keep = qe.regions_to_keep;

      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tfuzz_one [TAINT_AWARE_MODE] (fname: %s, is_initial_seed: %d): target_region %d, target_offset %d, target_len %d\n", queue_cur->fname, queue_cur->is_initial_seed, target_region, target_offset, target_len);
        fclose(fp);
      }

      n_fetched_taint_hints++;

      update_taint_fetch_count(queue_cur->fname, out_dir, n_fetched_taint_hints);
    }
    else {
      /* Select M2 randomly */
      u32 total_region = queue_cur->region_count;
      ...
      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tfuzz_one (fname: %s): M2_start_region_ID = UR(total_region) (%d)\n", queue_cur->fname, M2_start_region_ID);
        fclose(fp);
      }
      ...
      if (debug) {
        FILE *fp = fopen("debug/fuzzing.log", "a+");
        fprintf(fp, "\tfuzz_one [RANDOM_MODE] (fname: %s, is_initial_seed: %d): target_region %d, target_offset %d, target_len %d\n", queue_cur->fname, queue_cur->is_initial_seed, target_region, target_offset, target_len);
        fclose(fp);
      }

      n_fetched_random_hints++;
    }
  }
  ...
  for (it = kl_begin(kl_messages); it != kl_end(kl_messages); it = kl_next(it)) {
	...
	if (enable_taint_aware_mode && taint_aware_mode && checkpoint_strategy) {
    s32 res;
    static u32 prev_timed_out = 0;

    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: BEFORE 'res = write(fsrv_ctl_fd, &prev_timed_out, 4)) != 4'\n");
      fclose(fp);
    }
    ...
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: AFTER 'res = write(fsrv_ctl_fd, &prev_timed_out, 4)) != 4'\n");
      fclose(fp);
    }

    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: BEFORE 'res = read(fsrv_st_fd, &child_pid, 4)) != 4'\n");
      fclose(fp);
    }
    ...
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: AFTER 'res = read(fsrv_st_fd, &child_pid, 4)) != 4'\n");
      fclose(fp);
    }

    /* Reproduce the prefix and insert a forkserver checkpoint after that */
    if (send_over_network(1))
      return 1;
  }
...
#define FLIP_BIT(_ar, _b) do { \
  ...
  for (stage_cur = 0; stage_cur < stage_max && stage_cur < (!stage_max_par ? stage_max : stage_max_par); stage_cur++) {
    ...
    FLIP_BIT(out_buf+target_offset, stage_cur);

    if (enable_taint_aware_mode && taint_aware_mode && !is_like_with_middle_modification(orig_in, len, out_buf, len, stage_name, stage_cur, 0, target_len)) PFATAL("in_buf has not been rightly mutated");
    ...
    if (!dumb_mode && (stage_cur & 7) == 7) {
      ...
      if (stage_cur == stage_max - 1 && cksum == prev_cksum) {
        ...
        if (a_len < MAX_AUTO_EXTRA) a_collect[a_len] = out_buf[target_offset + stage_cur >> 3];
        a_len++;
  ...
  for (stage_cur = 0; stage_cur < stage_max && stage_cur < (!stage_max_par ? stage_max : stage_max_par); stage_cur++) {
    ...
    FLIP_BIT(out_buf+target_offset, stage_cur);
    FLIP_BIT(out_buf+target_offset, stage_cur + 1);
  ...
  for (stage_cur = 0; stage_cur < stage_max && stage_cur < (!stage_max_par ? stage_max : stage_max_par); stage_cur++) {
    ...
    FLIP_BIT(out_buf+target_offset, stage_cur);
    FLIP_BIT(out_buf+target_offset, stage_cur + 1);
    FLIP_BIT(out_buf+target_offset, stage_cur + 2);
    FLIP_BIT(out_buf+target_offset, stage_cur + 3);
...
#define EFF_SPAN_ALEN(_p, _l) (EFF_APOS((_p) + (_l) - 1) - EFF_APOS(_p) + 1)
  ...
  eff_map    = ck_alloc(EFF_ALEN(target_len));
  eff_map[0] = 1;

  if (EFF_APOS(target_len - 1) != 0) {
    eff_map[EFF_APOS(target_len - 1)] = 1;
    eff_cnt++;
  }
  ...
  for (stage_cur = 0; stage_cur < stage_max && stage_cur < (!stage_max_par ? stage_max : stage_max_par); stage_cur++) {
    ...
    out_buf[target_offset + stage_cur] ^= 0xFF;
  ...
  if (eff_cnt != EFF_ALEN(target_len) &&
      eff_cnt * 100 / EFF_ALEN(target_len) > EFF_MAX_PERC) {

    memset(eff_map, 1, EFF_ALEN(target_len));

    blocks_eff_select += EFF_ALEN(target_len);
  ...
  blocks_eff_total += EFF_ALEN(target_len);
  ...
  if (no_bitflip && target_len < 2) goto skip_bitflip;

  stage_name  = "bitflip 16/8";
  stage_short = "flip16";
  stage_cur   = 0;
  stage_max   = target_len - 1;
  ...
  for (i = 0; i < target_len - 1; i++) {
    ...
    *(u16*)(out_buf + target_offset + i) ^= 0xFFFF;
  ...
  if (no_bitflip && target_len < 4) goto skip_bitflip;
  ...
  for (i = 0; i < target_len - 3; i++) {
    ...
    *(u32*)(out_buf + target_offset + i) ^= 0xFFFFFFFF;
...
skip_bitflip:
  ...
  stage_short = "arith8";
  stage_cur   = 0;
  stage_max   = 2 * target_len * ARITH_MAX;
  ...
  for (i = 0; i < target_len; i++) {
    ...
    u8 orig = out_buf[target_offset + i];
    ...
    for (j = 1; j <= ARITH_MAX; j++) {
      ...
      if (!could_be_bitflip(r)) {
        ...
        stage_cur_val = j;
        out_buf[target_offset + i] = orig + j;
  ...
  if (no_arith && target_len < 2) goto skip_arith;

  stage_name  = "arith 16/8";
  stage_short = "arith16";
  stage_cur   = 0;
  stage_max   = 4 * (target_len - 1) * ARITH_MAX;

  orig_hit_cnt = new_hit_cnt;

  for (i = 0; i < target_len - 1; i++) {

    u16 orig = *(u16*)(out_buf + target_offset + i);
    ...
    for (j = 1; j <= ARITH_MAX; j++) {
      ...
      if ((orig & 0xff) + j > 0xff && !could_be_bitflip(r1)) {
        ...
        stage_cur_val = j;
        *(u16*)(out_buf + target_offset + i) = orig + j;
      ...
      if ((orig >> 8) + j > 0xff && !could_be_bitflip(r3)) {
        ...
        stage_cur_val = j;
        *(u16*)(out_buf + target_offset + i) = SWAP16(SWAP16(orig) + j);
      ...
      *(u16*)(out_buf + target_offset + i) = orig;
  ...
  if (no_arith && target_len < 4) goto skip_arith;

  stage_name  = "arith 32/8";
  stage_short = "arith32";
  stage_cur   = 0;
  stage_max   = 4 * (target_len - 3) * ARITH_MAX;

  orig_hit_cnt = new_hit_cnt;

  for (i = 0; i < target_len - 3; i++) {

    u32 orig = *(u32*)(out_buf + target_offset + i);
    ...
    for (j = 1; j <= ARITH_MAX; j++) {
      ...
      if ((orig & 0xffff) + j > 0xffff && !could_be_bitflip(r1)) {
        ...
        stage_cur_val = j;
        *(u32*)(out_buf + target_offset + i) = orig + j;
      ...
      if ((SWAP32(orig) & 0xffff) + j > 0xffff && !could_be_bitflip(r3)) {
        ...
        stage_cur_val = j;
        *(u32*)(out_buf + target_offset + i) = SWAP32(SWAP32(orig) + j);
      ...
      *(u32*)(out_buf + target_offset + i) = orig;
...
skip_arith:
  ...
  stage_short = "int8";
  stage_cur   = 0;
  stage_max   = target_len * sizeof(interesting_8);
  ...
  for (i = 0; i < target_len; i++) {
    ...
    for (j = 0; j < sizeof(interesting_8); j++) {
      ...
      out_buf[target_offset + i] = interesting_8[j];
  ...
  if (no_interest || target_len < 2) goto skip_interest;

  stage_name  = "interest 16/8";
  stage_short = "int16";
  stage_cur   = 0;
  stage_max   = 2 * (target_len - 1) * (sizeof(interesting_16) >> 1);
  ...
  for (i = 0; i < target_len - 1; i++) {
    ...
    for (j = 0; j < sizeof(interesting_16) / 2; j++) {
      ...
      if (!could_be_bitflip(orig ^ (u16)interesting_16[j]) &&
        ...
        *(u16*)(out_buf + target_offset + i) = interesting_16[j];
      ...
      if ((u16)interesting_16[j] != SWAP16(interesting_16[j]) &&
        ...
        *(u16*)(out_buf + target_offset + i) = SWAP16(interesting_16[j]);
  ...
  if (no_interest && target_len < 4) goto skip_interest;
  ...
  stage_short = "int32";
  stage_cur   = 0;
  stage_max   = 2 * (target_len - 3) * (sizeof(interesting_32) >> 2);
  ...
  for (i = 0; i < target_len - 3; i++) {
    ...
    for (j = 0; j < sizeof(interesting_32) / 4; j++) {
      ...
      if (!could_be_bitflip(orig ^ (u32)interesting_32[j]) &&
        ...
        *(u32*)(out_buf + target_offset + i) = interesting_32[j];
      ...
      if ((u32)interesting_32[j] != SWAP32(interesting_32[j]) &&
        ...
        *(u32*)(out_buf + target_offset + i) = SWAP32(interesting_32[j]);
...
skip_interest:
  ...
  stage_short = "ext_UO";
  stage_cur   = 0;
  stage_max   = extras_cnt * target_len;
  ...
  for (i = 0; i < target_len; i++) {
    ...
    for (j = 0; j < extras_cnt; j++) {
      ...
      if ((extras_cnt > MAX_DET_EXTRAS && UR(extras_cnt) >= MAX_DET_EXTRAS) ||
          extras[j].len > target_len - i ||
          !memcmp(extras[j].data, out_buf + target_offset + i, extras[j].len) ||
      ...
      memcpy(out_buf + target_offset + i, extras[j].data, last_len);
    ...
    memcpy(out_buf + target_offset + i, in_buf + target_offset + i, last_len);
  ...
  for (i = target_offset; i < target_offset + target_len; i++) {
    ...
    for (j = 0; j < extras_cnt; j++) {
      ...
      memcpy(ex_tmp, out_buf, target_offset);
      ...
      if (enable_taint_aware_mode && taint_aware_mode && !is_like_with_middle_modification(orig_in, len, ex_tmp, len + extras[j].len, stage_name, stage_cur, 0, target_len)) PFATAL("in_buf has not been rightly mutated");
...
skip_user_extras:
  ...
  stage_short = "ext_AO";
  stage_cur   = 0;
  stage_max   = MIN(a_extras_cnt, USE_AUTO_EXTRAS) * target_len;
  ...
  for (i = 0; i < target_len; i++) {
    ...
    for (j = 0; j < MIN(a_extras_cnt, USE_AUTO_EXTRAS); j++) {
      ...
      if (a_extras[j].len > target_len - i ||
          !memcmp(a_extras[j].data, out_buf + target_offset + i, a_extras[j].len) ||
      ...
      memcpy(out_buf + target_offset + i, a_extras[j].data, last_len);
...
havoc_stage:
  ...
  for (stage_cur = 0; stage_cur < stage_max && stage_cur < (!stage_max_par ? stage_max : stage_max_par); stage_cur++) {
    ...
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: %d/%d stage, use_stacking: %d\n", stage_cur, stage_max, use_stacking);
      fclose(fp);
    }
    ...
    for (i = 0; i < use_stacking; i++) {
      ...
      switch (rand) {
        ...
        case 0:
          ...
          FLIP_BIT(out_buf + target_offset, UR(temp_target_len << 3));
          break;
        ...
        case 1:
          ...
          out_buf[target_offset + UR(temp_target_len)] = interesting_8[UR(sizeof(interesting_8))];
          break;
        ...
        case 2:
          ...
          if (temp_target_len < 2) break;

          if (UR(2)) {

            *(u16*)(out_buf + target_offset + UR(temp_target_len - 1)) =
              interesting_16[UR(sizeof(interesting_16) >> 1)];

          } else {

            *(u16*)(out_buf + target_offset + UR(temp_target_len - 1)) = SWAP16(
              interesting_16[UR(sizeof(interesting_16) >> 1)]);
        ...
        case 3:
          ...
          if (temp_target_len < 4) break;

          if (UR(2)) {

            *(u32*)(out_buf + target_offset + UR(temp_target_len - 3)) =
              interesting_32[UR(sizeof(interesting_32) >> 2)];

          } else {

            *(u32*)(out_buf + target_offset + UR(temp_target_len - 3)) = SWAP32(
              interesting_32[UR(sizeof(interesting_32) >> 2)]);
        ...
        case 4:
          ...
          out_buf[target_offset + UR(temp_target_len)] -= 1 + UR(ARITH_MAX);
          break;
        ...
        case 6:
          ...
          if (UR(2)) {
            ...
            u32 pos = UR(temp_target_len - 1);

            *(u16*)(out_buf + target_offset + pos) -= 1 + UR(ARITH_MAX);
            ...
            *(u16*)(out_buf + target_offset + pos) =
              SWAP16(SWAP16(*(u16*)(out_buf + target_offset + pos)) - num);
        ...
        case 8:
          ...
          if (UR(2)) {
            ...
            u32 pos = UR(temp_target_len - 3);

            *(u32*)(out_buf + target_offset + pos) -= 1 + UR(ARITH_MAX);
            ...
            *(u32*)(out_buf + target_offset + pos) =
              SWAP32(SWAP32(*(u32*)(out_buf + target_offset + pos)) - num);
        ...
        case 10:
          ...
          out_buf[target_offset + UR(temp_target_len)] ^= 1 + UR(255);
          break;
        ...
        case 11 ... 12: {
            ...
            del_len = choose_block_len(temp_target_len - 1);

            del_from = UR(temp_target_len - del_len + 1);

            if (debug) {
              FILE *fp = fopen("debug/fuzzing.log","a+");
              fprintf(fp, "\toutbuf = 0x%lx, target_offset= %d, del_from = %d, del_len = %d, temp_target_len = %d, temp_len = %d\n", out_buf, target_offset, del_from, del_len, temp_target_len, temp_len);
              fclose(fp);
            }

            memmove(out_buf + target_offset + del_from, out_buf + target_offset + del_from + del_len,
                    temp_len - target_offset - del_from - del_len);

            temp_len -= del_len;
            temp_target_len -= del_len;
        ...
        case 13:
          ...
          if (temp_len + HAVOC_BLK_XL < MAX_FILE) {
            ...
            if (actually_clone) {
              ...
              clone_len  = choose_block_len(temp_target_len);
              clone_from = UR(temp_target_len - clone_len + 1);
            ...
            clone_to   = UR(temp_target_len);

            new_buf = ck_alloc_nozero(temp_len + clone_len);

            memcpy(new_buf, out_buf, target_offset);

            /* Head */

            memcpy(new_buf + target_offset, out_buf + target_offset, clone_to);

            /* Inserted part */

            if (actually_clone)
              memcpy(new_buf + target_offset + clone_to, out_buf + target_offset + clone_from, clone_len);
            else
              memset(new_buf + target_offset + clone_to,
                     UR(2) ? UR(256) : out_buf[target_offset + UR(temp_target_len)], clone_len);

            /* Tail */
            memcpy(new_buf + target_offset + clone_to + clone_len, out_buf + target_offset + clone_to,
                   temp_len - (target_offset + clone_to));

            ck_free(out_buf);
            out_buf = new_buf;
            temp_len += clone_len;
            temp_target_len += clone_len;
        ...
        case 14: {
            ...
            copy_len  = choose_block_len(temp_target_len - 1);

            copy_from = UR(temp_target_len - copy_len + 1);
            copy_to   = UR(temp_target_len - copy_len + 1);

            if (UR(4)) {

              if (copy_from != copy_to)
                memmove(out_buf + target_offset + copy_to, out_buf + target_offset + copy_from, copy_len);

            } else memset(out_buf + target_offset + copy_to,
                          UR(2) ? UR(256) : out_buf[target_offset + UR(temp_target_len)], copy_len);
        ...
        case 15: {
            ...
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
              ...
              if (extra_len > temp_target_len) break;

              insert_at = UR(temp_target_len - extra_len + 1);
              memcpy(out_buf + target_offset + insert_at, a_extras[use_extra].data, extra_len);
              ...
              insert_at = UR(temp_target_len - extra_len + 1);
              memcpy(out_buf + target_offset + insert_at, extras[use_extra].data, extra_len);
        ...
        case 16: {
            ...
            u32 use_extra, extra_len, insert_at = UR(temp_target_len + 1);
            u8* new_buf;
            ...
            if (!extras_cnt || (a_extras_cnt && UR(2))) {
              ...
              /* Head */
              memcpy(new_buf + target_offset, out_buf + target_offset, insert_at);

              /* Inserted part */
              memcpy(new_buf + target_offset + insert_at, a_extras[use_extra].data, extra_len);
              ...
              /* Inserted part */
              memcpy(new_buf + target_offset + insert_at, extras[use_extra].data, extra_len);

            }

            /* Tail */
            memcpy(new_buf + target_offset + insert_at + extra_len, out_buf + target_offset + insert_at,
                   temp_len - (target_offset + insert_at));
            ck_free(out_buf);
            out_buf   = new_buf;
            temp_len += extra_len;
            temp_target_len += extra_len;
        ...
        case 17: {
            ...
            temp_target_len = src_region_len;
            break;
          }
      ...
      if (enable_taint_aware_mode && taint_aware_mode && !is_like_with_middle_modification(orig_in, len, out_buf, temp_len, stage_name, stage_cur, rand, temp_target_len))
        PFATAL("in_buf has not been rightly mutated");     
...
abandon_entry:
  ...
  if (enable_taint_aware_mode && taint_aware_mode && checkpoint_strategy) {
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: BEFORE 'kill(checkpoint_forksrv_pid, SIGTERM)'\n");
      fclose(fp);
    }

    if (checkpoint_forksrv_pid > 0) kill(checkpoint_forksrv_pid, SIGTERM);
    checkpoint_forksrv_pid = 0;

    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: AFTER 'kill(checkpoint_forksrv_pid, SIGTERM)'\n");
      fclose(fp);
    }
    ...
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: BEFORE '((res = read(fsrv_st_fd, &status, 4)) != 4)'\n");
      fclose(fp);
    }
    ...
    if (debug) {
      FILE *fp = fopen("debug/fuzzing.log","a+");
      fprintf(fp, "\tfuzz_one: AFTER '((res = read(fsrv_st_fd, &status, 4)) != 4)'\n");
      fclose(fp);
    }
  }
  ...
  while ((sd_ent = readdir(sd))) {
    ...
    while ((qd_ent = readdir(qd))) {
      ...
      if (st.st_size && st.st_size <= MAX_FILE) {
        ...
        region_t *regions;
        u32 region_count;
        regions = (*extract_requests)(mem, st.st_size, &region_count, region_delimiter);
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -y sec        - timeout for the fuzzing session\n"
...
EXP_ST void setup_dirs_fds(void) {
  ...
  if (replay_mode) {
    ...
    const char *subdirs[] = { "queue", "hangs", "crashes" };

    for (int i = 0; i < 3; i++) {

      tmp = alloc_printf("%s/%s", out_dir, subdirs[i]);

      DIR *d = opendir(tmp);
      if (!d) PFATAL("Unable to open '%s'", tmp);

      struct dirent *entry;

      if (i == 0) {
        DIR *in_d = opendir(in_dir);
        if (!in_d) PFATAL("Unable to open '%s'", in_dir);

        while ((entry = readdir(in_d))) {
          if (entry->d_type != DT_REG) continue;

          char *file_path = alloc_printf("%s/%s", in_dir, entry->d_name);
          if (unlink(file_path) != 0) PFATAL("Unable to delete '%s'", file_path);
          ck_free(file_path);
        }
      ...
      while ((entry = readdir(d))) {
        if (entry->d_type != DT_REG) continue;
        if (!strcmp(entry->d_name, "README.txt")) continue;

        char *old_path = alloc_printf("%s/%s", tmp, entry->d_name);
        char *new_path = alloc_printf("%s/%s", in_dir, entry->d_name);

        if (rename(old_path, new_path) != 0)
          PFATAL("Unable to move '%s' to '%s'", old_path, new_path);
  ...
  if (sync_id && mkdir(sync_dir, 0777) && errno != EEXIST)
  ...
  if (replay_mode) {
    ...
    fprintf(plot_file, "# unix_time, paths_total, map_size, unique_crashes, unique_hangs, "
                      "stability, n_calibration\n");
                      /* ignore errors */    
  }
  else {
...
int main(int argc, char** argv) {
  warnf_output_file = fopen("warnings.log", "w");
  ...
  env_var = getenv("SEQUENCE_MINIMIZATION");
  if (env_var && !strcmp(env_var, "1")) {
    sequence_minimization = 1;
  }

  region_delimiter = getenv("REGION_DELIMITER");
  if (!region_delimiter) {
    PFATAL("REGION_DELIMITER environment variable is not set");
  }

  if (setenv("EXEC_MODE", "AFLNET", 1) != 0) {
    perror("setenv");
    return EXIT_FAILURE;
  }
  ...
  shm_unlink(SHARED_CHILD_RETVAL);
  shm_fd = shm_open(SHARED_CHILD_RETVAL, O_CREAT | O_RDWR, 0666);
  if (shm_fd == -1) {
      perror("shm_open");
      exit(EXIT_FAILURE);
  }
  ...
  size_t shmem_size = NUM_TRACES * sizeof(trace_t);
  if (ftruncate(shm_fd, shmem_size) == -1) {
      perror("ftruncate");
      exit(EXIT_FAILURE);
  }

  cur_crash_traces = mmap(NULL, shmem_size, PROT_READ | PROT_WRITE, MAP_SHARED, shm_fd, 0);
  if (cur_crash_traces == MAP_FAILED) {
      perror("mmap");
      exit(EXIT_FAILURE);
  }

  memset(cur_crash_traces, 0, shmem_size);
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:T:dnCB:S:M:x:QN:D:w:e:P:Eq:s:RFc:l:y:HXY")) > 0)
    ...
    switch (opt) {
      ...
      case 'y': /* fuzzing session timeout */
        if (fuzz_tmout) FATAL("Multiple -y options not supported");

        if (sscanf(optarg, "%u", &fuzz_tmout) < 1 || optarg[0] == '-') FATAL("Bad syntax used for -y");
      ...
      case 'w': /* child timeout determining time waited for each response */
        if (child_tmout) FATAL("Multiple -w options not supported");

        if (sscanf(optarg, "%u", &child_tmout_secs) < 1 || optarg[0] == '-') FATAL("Bad syntax used for -w");
        child_tmout = 1;
        break;
      ...
      case 'H':
        if (taint_aware_mode) FATAL("Multiple -Z options not supported");
        taint_aware_mode = 1;
        break;
      ...
      case 'Y':
        if (replay_mode) FATAL("Multiple -Y options not supported");
        replay_mode = 1;
        break;

      case 'X':
        if (checkpoint_strategy) FATAL("Multiple -X options not supported");
  ...
  env_var = getenv("AFL_NO_ARITH");
  if (env_var) {
    no_arith = atoi(env_var);
  }
  env_var = getenv("AFL_NO_BITFLIP");
  if (env_var) {
    no_bitflip = atoi(env_var);
  }
  env_var = getenv("AFL_NO_INTEREST");
  if (env_var) {
    no_interest = atoi(env_var);
  }
  env_var = getenv("AFL_NO_USER_EXTRAS");
  if (env_var) {
    no_user_extras = atoi(env_var);
  }
  env_var = getenv("AFL_NO_EXTRAS");
  if (env_var) {
    no_extras = atoi(env_var);
  }
  env_var = getenv("AFL_SHUFFLE_QUEUE");
  if (env_var) {
    shuffle_queue = atoi(env_var);
  }
  env_var = getenv("AFL_CALIBRATION");
  if (env_var) {
   calibration = atoi(env_var);
  }

  if (calibration == -1) {
    OKF("Auto-calibration activated");
  }
  ...
  remove("/STAFF/wait_for_container_init");
    ...
    while (1) {
      ...
      if (!enable_taint_aware_mode) {
        ...
        while(!selected_seed || selected_seed->region_count == 0) {
          ...
          if (debug) {
            FILE *fp = fopen("debug/fuzzing.log", "a+");
            fprintf(fp, "\tSTATE_AWARE_MODE: target_state_id = %d, selected_seed->fname = %s\n", target_state_id, selected_seed->fname);
            fclose(fp);
          }
        }
